...
\cz{
    While many researchers struggled to improve complexity analysis for the standard trust-region method, \citet{cartisComplexitySteepestDescent2010,cartisComplexityBoundsSecondorder2012} showed that its complexity rate is at least $O(\epsilon^{-2})$,
...
\cz{
    ...
    \begin{center}
        \emph{Can the trust-region-type method be accelerated for convex optimization?}
    \end{center}
    We remark that beyond cubic regularization, other accelerated second-order approaches have been developed~\cite{monteiro2013accelerated,carmon2022optimal,doikov2020contracting}. However, all the above methods stick with \emph{unconstrained subproblems}, and thus can be safely dealt in \emph{primal space}.
    However, trust-region-type methods remain obviously absent from this landscape in the presence of the ball constraint of trust-region subproblems, where suitable treatment for the \emph{dual variable} is required.
}
...
\end{equation*}
and therefore $\mu_k = \frac{M}{2}\|d_k\|$. 

Fortunately, although this favorable regularity condition is not inherently guaranteed for trust-region-type methods, there is a way to overcome this challenge. Specifically, with careful choice of $(\sigma_k,r_k)$, the progress made in the subproblem formulation~\eqref{eq.newTR} can be interpreted in two distinct ways. As demonstrated in the following two lemmas: when $\lambda_k>0$, we achieve a sufficient decrease in the function value; when $\lambda_k = 0$, we establish a bound on the gradient norm for the next iteration.
...
\begin{lemma}
    ...
    Suppose Assumption~\ref{assm.main assm} holds, and \eqref{eq.second-order update} holds for some $\mu_k$ with $\mu_k\geq M \|d_k\|$, then we have
    \begin{equation}
...
\begin{proof}
    ...
    \end{aligned}
    \end{equation*}
    The second line is due to $\mu_k\geq M\|d_k\|$, dividing both sides by $2\mu_k$ gives \eqref{eq.inner product g d appendix}. Further, when $\mu_k \leq 2M\|d_k\|$, the above gives
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    Plug \eqref{eq.optcond firstorder} into the above we have \eqref{eq.function decrease appendix}. Note that when $\lambda_k>0$, from \eqref{eq.optcond coml slack} we have $\|d_k\| = r_k$, from \eqref{eq.function decrease appendix}, we have
    \begin{equation*}
        \begin{aligned}
...
\begin{lemma}
    ...
    Suppose \autoref{assm.main assm} holds and $d_k,\lambda_k= \operatorname{SA}(y_k,\sigma_k,r_k)$ , then we have
    \begin{equation}
        \label{eq.bound next gradient appendix}
        \|\nabla f(y_k+d_k)\| \leq \frac{M}{2}r_k^2+(\sigma_k+\lambda_k)r_k.
    \end{equation}
\end{lemma}
\begin{proof}
    From \eqref{eq.second-order exp}, we have
    \begin{equation*}
        \|\nabla f(y_k+d_k)- \nabla f(y_k) -\nabla ^2 f(y_k) d_k\| \leq \frac{M}{2}\|d_k\|^2.
    \end{equation*}
    By \eqref{eq.optcond firstorder} and the above, we have
    \begin{equation*}
        \|\nabla f(y_k+d_k)+(\sigma_k+\lambda_k) d_k\| \leq \frac{M}{2}\|d_k\|^2.
    \end{equation*}
    Note $\|d_k\|\leq r_k$, apply the triangle inequality and rearrange items we can derive \eqref{eq.bound next gradient appendix}.
\end{proof}
...
\end{proof}
\paragraph{Proof to Lemma~\ref{lem.inner loop bound next gradient}}
\begin{lemma}
    \label{lem.inner loop bound next gradient appendix}
    Suppose Assumption~\ref{assm.main assm} holds, if in the $j$-th iteration, Algorithm~\ref{alg.rr} does not output $d_k^{(j)}$ or terminate, then
    \begin{equation}
