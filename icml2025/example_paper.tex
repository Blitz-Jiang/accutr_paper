...
\begin{equation}
    \label{eq.classic tr subproblem} \begin{aligned} \min_{d \in \mathbb{R}^n} \  & m_k^{\text{TR}}(d):= \nabla f(x_k)^T d+\frac{1}{2}d^T \nabla^2 f(x_k) d, \\ \operatorname{s.t.} \ &\|d\|\le \Delta_k.
    \end{aligned}
\end{equation}
The solution to this subproblem is given by $d_k$ and $\lambda_k$ is the dual variable to the ball constraint.
...
\cz{
    ...
    However, trust-region-type methods remain obviously absent from this landscape in the presence of the ball constraint of trust-region subproblems, where suitable treatment for the \emph{dual variable} $\lambda_k$ is required.
}

\paragraph{Our approach}
To illustrate the difficulty, we begin by introducing a key property required for acceleration, referred to as the \emph{regularity condition} (Definition~\ref{def.regularity condition}).
...
Accelerated algorithms, such as the cubic regularized Newton method \cite{nesterov2008accelerating} (and many others \cite{monteiro2013accelerated,carmon2022optimal,doikov2020contracting}), build on unconstrained regularized subproblems that explicitly enables the step size $\|d_k\|$ to scale as some monomial of the gradient norm $\|\nabla f(x_k)\|$. Consequently, the regularity condition is naturally satisfied for such algorithms.
By contrast, trust-region subproblems are solved by a feedback control, leading to complex dependencies of the step size and gradient norm on the posterior multiplier $\lambda_k$ \cite{nocedal1999numerical,cartis2022evaluation}. As a result, trust-region methods do not consistently satisfy the regularity condition and therefore cannot ensure the function value decrease required by the estimating sequence technique.

Therefore, to accelerate trust-region methods, we employ ... based on a new subproblem introduced in \cite{jiang2024nonconvexityuniversaltrustregionmethod}. The resulting accelerated trust-region method \atr{} achieves a global iteration complexity of $O(\epsilon^{-1/3})$.
Shortly we show the progress of \atr{} alternates between two distinct forms: reducing the function value or decreasing the gradient norm; to the best of our knowledge, the latter form is the first time witnessed in these methods.
\begin{table*}[t]
