...
For convenience, we denote the exact primal-dual solution pair of \eqref{eq.newTR} as $(d,\lambda) = \operatorname{SA}(x, \sigma, r)$, where SA stands for Subproblem for Acceleration.

As can be observed, among all the above conditions, \eqref{eq.optcond firstorder} is the decisive condition for step $d$. Similar to the update rule in~\eqref{eq.optcond firstorder}, most second-order methods update the step at $x$ in the following form:
\begin{equation}
    \label{eq.second-order update}
    \left (\nabla^2 f(x) + \mu I \right )d = -\nabla f(x),
\end{equation}
where $\mu$ is a regularizer that varies depending on the specific method and is crucial to verify if step $d$ satisfies the regularity condition. In particular, to verify whether the regularity condition holds, the relation $\mu = \Theta(\|d\|)$ serves as a sufficient condition.
\begin{lemma}
