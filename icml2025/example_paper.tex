...
\begin{center}
    \emph{Can the trust-region-type method be further accelerated?}
\end{center}
However, addressing this question poses significant challenges due to the inherently constrained nature of trust-region subproblems.
...
The regularity condition plays a crucial role in enabling the estimating sequence technique, which serves as a key tool for accelerating algorithms. This technique relies on establishing a recursive relationship involving the function value across iterations, ensuring consistent cumulative progress that ultimately leads to acceleration. The regularity condition is essential for maintaining this recursive relationship, as it guarantees the required decrease in the function value throughout the iterations.

Unlike algorithms that satisfy the regularity condition, trust-region subproblems introduce posterior multipliers, leading to complex dependencies among the step size, multiplier, and gradient norm. As a result, trust-region methods do not consistently satisfy the regularity condition and therefore cannot ensure the function value decrease required by the estimating sequence technique. Instead, their progress alternates between two distinct forms: reducing the function value or decreasing the gradient norm, reflecting a fundamentally different mechanism of iteration.
