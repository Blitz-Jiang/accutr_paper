...
\begin{abstract}
    ...
    \cz{
        Trust-region-type methods are fundamental schemes in optimization due to due to their excellent practical performance. However, the ball-constrained subproblems pose significant obstacles for complexity analysis in convex settings, making it a daunting task for devising acceleration schemes. In this paper, we introduce a novel acceleration framework for trust-region-type methods, by a few novel techniques to refine the essential  characteristics of TRS. This leads to the development of the \emph{first} accelerated trust-region algorithm with an improved global iteration complexity of $O(\epsilon^{-1/3})$. Numerical experiments demonstrate that the proposed algorithm achieves performance comparable to Nesterov's accelerated cubic Newton method, while consistently outperforming non-accelerated trust-region-type methods and the cubic Newton method.
    }
\end{abstract}
...
Recently, global complexity has emerged as a crucial metric for evaluating optimization algorithms. Early studies established that the standard trust-region method converges to stationary points with an iteration complexity of $O(\epsilon^{-2})$, matching that of first-order methods \cz{\cite{nesterov_lectures_2018}} but being suboptimal in terms of second-order methods \cz{\cite{carmon2021lower}}.
\cz{
    While many researchers struggled to improve complexity analysis for the standard trust-region method, \cite{cartisComplexitySteepestDescent2010,cartisComplexityBoundsSecondorder2012} showed that its complexity rate is at least $O(\epsilon^{-2})$,
    which has led to various new variants of trust-region methods to achieve better global complexity results.
    For example, for finding $\epsilon$-approximate second-order stationary points of nonconvex functions, several trust-region-type variants achieve the iteration complexity of $O(\epsilon^{-3/2})$~\cite{luenberger_linear_2021, curtis2017trust, curtis2021trust, hamad2022consistently, hamad2024simplepracticaladaptivetrustregion}, albeit with different algorithmic design and analytical techniques. For details on their approaches, we refer readers to their respective works. For convex optimization, \citet{jiang2024nonconvexityuniversaltrustregionmethod} proposed a universal trust-region method with an iteration complexity of $O(\epsilon^{-1/2})$ very recently.
}

\cz{
    We acknowledge the significant contributions of the above results; however, there remains considerable room for trust-region methods. In convex optimization, acceleration techniques~\cite{nesterovMethodSolvingConvex1983a,nesterov_lectures_2018} have emerged as a critical aspect of the research, due to theoretical and practical successes of accelerated first-order methods in machine learning~\cite{lan_first-order_2020,lin2020accelerated}.
}

This growing interest extends to accelerating the second-order methods. \citet{nesterov2008accelerating} showed that it is possible to accelerate the cubic regularized Newton method~\cite{nesterov2006cubic}, which relies on a different \emph{unconstrained} cubic-regularized subproblem of the following form:
\begin{equation}
    \label{eq.cr update}
    \min_{d \in \mathbb{R}^n} \  m_k^{\text{CR}}(d):=\nabla f(x_k)^T d+\frac{1}{2}d^T \nabla^2 f(x_k) d+\frac{M}{6}\|d_k\|^3,
\end{equation}
where $M>0$ is defined in Assumption~\ref{assm.main assm}. This method holds significant promise due to its ability of handling nonconvex as well as convex problems. Specifically, while achieving iteration complexities of $O(\epsilon^{-3/2})$ for nonconvex problems and $O(\epsilon^{-1/2})$ for convex problems, it can be further accelerated to $O(\epsilon^{-1/3})$ in the convex setting. This marks a stark contrast to the current limitations of trust-region-type methods.
...
\cz{
    ...
    \begin{center}
        \emph{Can the trust-region-type method be accelerated?}
    \end{center}
    We remark that beyond cubic regularation, other accelerated second-order approaches have been developed~\cite{monteiro2013accelerated,carmon2022optimal,doikov2020contracting}. However, all the above methods stick with \emph{unconstrained subproblems}, and thus can be safely delt in \emph{primal space}.
    However, trust-region-type methods remain obviously absent from this landscape in presence of the ball constraint of trust-region subproblems, where suitable treatment for the \emph{dual variable} is required.
}
...
\cz{
    To the best of our knowledge, the work \cite{carmon2018accelerated} is the only one that addresses accelerating algorithms with a constrained subproblem. Their method employs a subproblem that includes a ball constraint with a fixed radius $r > 0$:
    \begin{equation}
...
In this section, we introduce the Accelerated Trust-Region (ATR) method (Algorithm~\ref{alg.accelerated utr}), along with its auxiliary subroutines (Algorithm~\ref{alg.rr} and Algorithm~\ref{alg.bisection}), and provide an explanation of the intuition behind their design.
