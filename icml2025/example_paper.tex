...
In this paper, we consider the unconstrained optimization problem \begin{equation} \label{eq.main problem} \min_{x\in \mathbb{R}^n}~ f(x), \end{equation} where $f:\mathbb{R}^n\to \mathbb{R}$ is convex with its Hessian being Lipschitz continuous. The standard trust-region method operates on the local quadratic approximation of the objective function at point $x\in\mathbb{R}^n$ within a trust-region ball:
\begin{equation}
...
The wave of research on accelerated algorithms has already reached the domain of second-order methods. \citet{nesterov2008accelerating} showed that it is possible to accelerate the cubic regularized Newton method~\cite{nesterov2006cubic}. This method holds significant promise due to its ability to handle convex problems. Specifically, its iteration complexity can be further accelerated to $O(\epsilon^{-1/3})$ from $O(\epsilon^{-1/2})$ for convex problems. This marks a stark contrast to the current limitations of trust-region-type methods.

Beyond cubic regularization, other accelerated second-order approaches have been developed~\cite{monteiro2013accelerated,doikov2020contracting,carmon2022optimal,kovalev2022first}. However, all the above methods stick with \emph{unconstrained subproblems} and thus can avoid the sophisticated discussion over the multiplier. The above algorithms have also spawned a series of studies, each focusing on different aspects, to name a few, see~\citet{jiang2020unified,chen2022accelerating,lin2022explicit,agafonov2023inexact,agafonov2024advancing,jiang2024accelerated,huang2024inexact}.
...
To illustrate the difficulty of accelerating trust-region-type methods, we begin by revisiting a key property required for acceleration, referred to as the \emph{regularity condition} (Definition~\ref{def.regularity condition}).
...
Apparently, among all the above conditions, \eqref{eq.optcond firstorder} forms the decisive computations of step $d$ and also plays a role in determining whether the algorithm can be accelerated. In fact, most second-order methods update the step at $x$ in a similar form as follows:
\begin{equation}
    \label{eq.second-order update}
    \left (\nabla^2 f(x) + \mu I \right )d = -\nabla f(x),
\end{equation}
where $\mu$ is a regularizer that varies among different methods. 

Next, we discuss the relationship between the \eqref{eq.second-order update} and acceleration in earlier literature. When it comes to acceleration, an unavoidable proposition is the regularity condition, which is mentioned earlier and is the desirable condition for second-order methods to be accelerated.
...
To verify whether the regularity condition holds, here is a commonly used sufficient condition based on the ratio of the regularizer $\mu$ and the step size $\|d\|$:
...
Guaranteed by Lemma~\ref{lem.inner product g and d}, the regularity condition always holds for the cubic Newton method, which relies on a different unconstrained cubic regularized subproblem of the following form:
\begin{equation}
    \label{eq.cr update}
    \min_{d\in\mathbb{R}^n}\ m^{\text{CR}}(d):=\nabla f(x)^d+\frac{1}{2}d^T\nabla^2 f(x)d+\frac{M}{6}\|d\|^3.
\end{equation}
This is because of the optimality condition of the subproblem \eqref{eq.cr update} gives
\begin{equation*}
    \left (\nabla^2 f(x) + \frac{M}{2}\|d\| I \right )d = -\nabla f(x),
\end{equation*}
and therefore $\mu = \frac{M}{2}\|d\|$.
% In convex optimization, the second-order condition \eqref{eq.optcond secondorder} readily holds, and most of the efforts are paid to  a trial-and-error process of $\mu$ to justify the regularity condition \eqref{eq.inner product g d advanced}. Instead of caculating an extra gradient $\nabla f(x+d)$, one observation is that a stable ratio $\mu / \|d\|$ serves as a sufficient condition for the regularity condition \eqref{eq.inner product g d advanced}.
% to verify whether the regularity condition holds, the relation $\mu = \Theta(\|d\|)$ serves as a sufficient condition.

However, in the case of the trust-region-type subproblems, such ratio $\frac{\mu}{\|d\|}$ is not that stable. Take our accelerated trust-region subprobem~\eqref{eq.newTR} as an example, we have $\mu = \sigma + \lambda$, where $\lambda$ is the posterior multiplier. The introduction of the multiplier $\lambda$ may cause the trust-region step to fail to satisfy the regularity condition.
\begin{figure}[ht]
    \centering
...
As discussed in the previous section, we must capture the essential characteristics of the subproblem to design an algorithm that can accelerate it even when the regularity condition does not hold.

Fortunately, although this favorable regularity condition is not inherently guaranteed for trust-region-type methods, they possess other exploitable characteristics that can serve as an alternative. Specifically, with careful choice of $(\sigma,r)$, the progress made in the subproblem formulation~\eqref{eq.newTR} can be interpreted in two distinct ways. As demonstrated in the following two lemmas: when $\lambda>0$, we achieve a sufficient decrease in the function value; when $\lambda = 0$, we establish a bound on the gradient norm for the next iteration.
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE $d_k,ET = \operatorname{LD}(y_k,d_k)$ \hfill\texttt{\# Algorithm 2}
        \IF{$ET$}
...
\begin{algorithm}[ht]  \caption{Local Detection for ATR (LD)}\label{alg.rr}
    \begin{algorithmic}[1]
        ...
        \STATE \textbf{output} $d_k^{(j-1)}$, $ET=\text{True}$
        \ENDIF
        ...
        \STATE \textbf{output } $d_k^{(j)}, ET=\text{False}$ \label{line.zero multiplier 1}
        ...
        \STATE \textbf{output } $d_k^{(j)}, ET=\text{False}$
...
We begin by clarifying Algorithm~\ref{alg.accelerated utr}, which governs the recursive relations in \eqref{eq.es relation 1} and \eqref{eq.es relation 2}. Following the guidance provided by Lemma~\ref{lem.function decrease} and Lemma~\ref{lem.bound next gradient}, a natural approach is to classify the iterations into two cases and handle them separately (\textbf{Case 1} and \textbf{Case 2} shown in Algorithm~\ref{alg.accelerated utr}). In these two scenarios, we adopt distinct strategies for updating the elements $x_k$ and $v_k$ in the estimating sequence to ensure that either the recursive relation smoothly transitions to the next iteration, or the algorithm terminates early (the flag $ET$ stands for early terminate). This distinction is the key difference between our algorithm and other acceleration frameworks.

Specifically, in \textbf{Case 1} of Algorithm~\ref{alg.accelerated utr}, the multiplier $\lambda_k>0$, which means that the constraint is active and our choice of $\sigma_k,r_k$ has brought a sufficient decrease in function value for the recursive relation to pass down to the next iterate:
...
However, \textbf{Case 2} of \Cref{alg.accelerated utr} calls for different processing. We design a Local Detection (LD) process (\Cref{alg.rr}) to deal with this case. Specifically, we use Algorithm~\ref{alg.rr} to investigate the local property of the current iterate $y_k$, whose goal is twofold.

On one hand, in \textbf{Phase 1} of Algorithm~\ref{alg.rr}, a distinctive feature is that the gradient norm of the next iterate will keep contracting as long as the multiplier $\lambda_k^{(j)}=0$. Therefore, intuitively, we want to use such contraction to speed up the convergence. In particular, in Algorihtm~\ref{alg.rr} we use the notation $\xi^{(j)}$ to keep track of such contraction and serve as the parameter for the next iteration.
\begin{corollary}
...
The logic of the analysis is that in Algorithm~\ref{alg.accelerated utr}, if the early terminate occurs before $K_\epsilon = O(\epsilon^{-1/3})$-th iteration, then there is nothing to prove. If it does not occur before $K_\epsilon = O(\epsilon^{-1/3})$-th out iteration, the estimate sequence technique can guarantee that we find an approximate solution.
...
\begin{lemma}
    ...
    At the $k$-th iteration of Algorithm~\ref{alg.accelerated utr}, where $k\in\mathcal{Z}$, the number of linear equations solved during the bisection search procedure is at most $O(\log \frac{\|\nabla f(y_k)\|}{\epsilon})$.
\end{lemma}

As observed, both \textbf{Phase 1} and \textbf{Phase 2} have a worst-case complexity of $O\left(\log\left(\frac{\|\nabla f(y_k)\|}{\epsilon}\right) \right)$, in \textbf{Phase 1} we need to solve trust-region subproblems while in \textbf{Phase 2} we only need to solve linear equations. We remark that it is more expensive to solve trust-region subproblems than linear equations. Therefore, the complexity of \textbf{Phase 2} is dominated by the complexity of \textbf{Phase 1}. Also, all these trust-region subproblems and linear equations share the same Hessian and gradient. This means if we use eigenvalue decomposition, the decomposition is a one-time effort.
...
\begin{proof}
    ...
    \begin{equation*}
        4M\|d\| \langle \nabla f(x+d),-d \rangle\geq 2\mu \langle \nabla f(x+d),-d \rangle\geq \|\nabla f(x+d)\|^2 +\frac{3}{4}\mu^2 \|d\|^2  \geq \|f(x+d)\|^2 +\frac{3}{4} M^2 \|d\|^4,
    \end{equation*}
    which yields
    \begin{equation*}
...
\end{proof}
Till now we have finished proof for the recursive relation in the estimating sequence technique. The $O(\epsilon^{-1/3})$ iteration complexity is a consequence of the recursive relation and the fact $A_k = \Omega(k^3)$.
\end{document}
