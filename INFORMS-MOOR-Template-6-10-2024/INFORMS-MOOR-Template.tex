...
In addition to local convergence, another important criterion for evaluating the effectiveness of optimization methods is their non-asymptotic global oracle complexity, \cz{namely, the number of oracle calls (i.e., second-order subproblems) required to obtain an approximate solution~\cite{cartis2022evaluation}.}  Over the past decades, a few SOMs have been developed with such emphases ~\cite{nesterov2006cubic,mishchenko2023regularized,jiang2026beyond,he2025homogeneous,curtis2021trust,he2023newton,cartis2011adaptive,cartis2011adaptive_i,curtis2017trust}.
% In convex optimization, some of these methods can be further improved through acceleration techniques.
\cz{In the convex setting, several of these methods admit further improvements.}

In this context, two major approaches have emerged. \cz{The first is the accelerated cubic regularized Newton (CRN) method~\cite{nesterov2008accelerating}, which builds upon the cubic regularization oracle~\cite{nesterov2006cubic} and achieves a global oracle complexity of $O(\epsilon^{-1/3})$.}
\cz{
    The second is the accelerated Newton proximal extragradient (A-NPE) method~\cite{monteiro2013accelerated}, which attains an oracle complexity of $\tilde{O}(\epsilon^{-2/7})$~ using second-order oracles satisfying certain error-bound conditions.
}
These developments \cz{are largely built upon} the estimating sequence technique~\cite{nesterov1983method,baes2009estimate} and have inspired a wide range of enhancements~\cite{jiang2021optimal,chen2022accelerating,lin2022explicit,agafonov2023inexact,agafonov2024advancing,jiang2024accelerated,huang2024inexact,doikov2020contracting}.
\cz{More recently, the logarithmic factor in the oracle complexity of A-NPE was eliminated, yielding the optimal complexity of} $O(\epsilon^{-2/7})$~\cite{carmon2022optimal,kovalev2022first}. In fact, these acceleration techniques are broadly applicable and are by no means limited to the domain of SOMs~\cite{xu2017accelerated,xu2018accelerated,lan2012optimal,ghadimi2016accelerated}.


However, it is challenging to balance the global and local efficiency of second-order optimization algorithms. \cz{While the classical Newton's method possesses quadratic local convergence,} it lacks global guarantees and often diverges when the starting point is far from the optimal solution~\cite{more1982newton,dennisjr1996numerical}. In contrast, stronger worst-case complexity results of accelerated SOMs often come at the cost of local efficiency.
...
Our theoretical findings are corroborated by numerical experiments. On a global scale, \cz{the accelerated trust-region methods and the accelerated cubic regularized Newton method~\cite{nesterov2008accelerating}} consistently outperform the non-accelerated \cz{counterparts~\cite{jiang2026beyond,nesterov2006cubic}}. Locally, \Cref{alg.accelerated utr} exhibits quadratic convergence, and \atrms{} cannot retain superlinear convergence near the solution, similar to the empirical conclusions in \cite{carmon2022optimal,jiang2024accelerated,huang2024inexact}. These findings confirm the necessity of local detection \cz{for retaining local quadratic convergence in TR-type methods.}
...
\begin{assumption}
    ...
    \end{equation}
\end{assumption}
{\color{red} Note that \Cref{assm.bounded hessian} is equivalent to the gradient of the objective function being Lipschitz.} 
...
The case $\lambda_k > 0$ typically indicates that the current iterate is still far from a local optimum. In this regime, as shown later in \Cref{lem.ratio pass nonzero}, the overall regularization parameter $\mu_k$ becomes automatically proportional to the step size, which justifies the updates of the estimating sequence in the acceleration framework.
...
\Halmos \endproof
To maintain the flow of the complexity analysis, we temporarily assume the output of R\&B of \Cref{alg.local detection} satisfies~\eqref{eq.output of alg2} as well, i.e., \eqref{eq.output of alg2} holds for $k\in\mathcal{Z}$, \cz{whenever ET=False (we will verify this in \Cref{lem.bound initial interval})}. Now we show that the relation \eqref{eq.es relation} holds during the update of \Cref{alg.accelerated utr}.
...
\begin{theorem}
    ...
    hold, and suppose that \Cref{eq.output of alg2} is satisfied at every non-terminated
...
\begin{corollary}
    ...
    iteration. Then it takes $O\left(\frac{M^{1/3}D_0}{\epsilon_f^{1/3}}\right)$ iterations for \Cref{alg.accelerated utr} to find $\epsilon_f$-function-value solutions as in \Cref{def.approximate solution}.
\end{corollary}
}
\begin{comment}
...
For any $r$, the existence of such $\mu$ follows from $(d,\mu)=\trp(y,0,r)$ and $\eqref{eq.optcond firstorder}$. For uniqueness, let $H:=\nabla^2 f(y)\succeq0$ and $g:=\nabla f(y)$. Since $H+\mu I\succ0$ for every $\mu\in[\mu_-,\mu_+]$, write
\[
    H=Q\operatorname{diag}(h_1,\ldots,h_n)Q^\top,
    \qquad Q=[q_1,\ldots,q_n],
\]
where $Q$ is orthogonal and $h_i\geq0$.
Then
\[
    r = \bigl\|(H + \mu I)^{-1}g\bigr\|
    = \sqrt{\sum_{i=1}^n \frac{\beta_i^2}{(h_i + \mu)^2}},
\]
where $\beta_i:=q_i^\top g$. Since $h_i\geq0$ and $\mu>0$, $r_y(\mu)$ is continuous and strictly decreasing on $[\mu_-,\mu_+]$.
...
\end{lemma}
\end{comment}
\subsubsection{Number of oracle calls of \Cref{alg.local detection}}\label{sec.line search}
\cz{We now estimate the number of oracle calls in \Cref{alg.local detection}.
    ...
    In this part of analysis, we may assume that $\|d_+\| > \min\{\frac{\epsilon_g}{2\kappa_H},\sqrt{\frac{\epsilon_g}{M}}\}$; otherwise, it ends at {Track 0}. Furthermore, since we also limit the calls in {Track~1}, we only have to estimate number of oracle calls in Track 2 (R\&B), i.e., the complexity of bisection over radius $r$ on the interval $[r_-,r_+]$. % to make \eqref{eq.output of alg2} hold.
...
%     \end{lemma}
% }%
The following lemma validates the choices of the bracketing points at line~\ref{line.left bracketing point} of \Cref{alg.local detection}.
\begin{lemma}
    \label{lem.valid bracketing}
    In \textnormal{R\&B} of \Cref{alg.local detection}, when the bisection begins, we have
    \begin{equation*}
...
\begin{lemma}
    \label{lem.bound initial interval}
    Suppose \Cref{assm.lipschitz} and \Cref{assm.bounded hessian} hold. At the $k$-th iteration of \Cref{alg.accelerated utr}, the R\&B procedure finds a solution pair with \eqref{eq.output of alg2} holds within the oracle complexity of 
    \begin{equation}
        \label{eq.complexity bisection}
        O\left(\log\tfrac{\kappa_H+\mu_+}{\epsilon_g}\right).
    \end{equation}
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
Note $\nabla^2 f(y_k)\succ \frac{\nu}{3} I$ as analyzed in \eqref{eq.psd xk}.
\begin{equation*}
    \begin{aligned}
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1]
        \STATE{\textbf{input:} $x_0=v_0\in \mathbb{R}^n$, \ $A_0 =0$, \ R\&B search threshold $\theta>1$, \ $0<\eta<1$, damping parameter $0<\gamma \leq \frac{1}{\theta}$, stationarity tolerance $\epsilon_g>0$}
        \FOR{$k=0, 1, 2, \ldots$}
...
\(
    c_\gamma:=\frac{4}{\sqrt{3\gamma}}+1, D_0:=\|x_0-x^*\|.
\)
We assume that Algorithm~\ref{alg.ms accelerated utr} is supplied with a finite certified bound $G_0>0$ satisfying
\begin{equation}
    \label{eq.certified G0}
    G_0\geq c_\gamma\|\nabla^2 f(x^*)\|D_0+\frac{M}{2}c_\gamma^2D_0^2.
\end{equation}
As proved in \Cref{lem.bounded iterate}, this choice guarantees
\(
\|\nabla f(y_k(\sigma))\|\leq G_0
\)
for every generated iterate and every $\sigma>0$.
In R\&B~(\Cref{alg.bisection ms}), the search task initializes the bracketing points at line~\ref{line.def G0} using this bound.
...
\end{equation*}
By canceling the common terms on both sides (telescoping sum), it simplifies to
\begin{equation*}
...
\begin{lemma}
    ...
    Suppose Assumptions~\ref{assm.lipschitz} and~\ref{assm.solvable} hold, and \eqref{eq.relation lambda sigma} holds for iterations $i=0,\ldots,k-1$. Then, for any $k\geq 1$, we have
    \begin{equation}
...
\end{theorem}
\subsubsection{\cz{Number of oracle calls of \Cref{alg.bisection ms}}}\label{sec.validty of bisection}
...
The below \Cref{lem.reduce line search} means the analysis of auxiliary function $\psi(\sigma,y(\sigma))$ can be simplified by focusing solely on $\sigma$, reducing the search procedure into a one-dimensional problem. The proof of \Cref{lem.reduce line search} can be found in \Cref{sec.appendix 2}.
\begin{lemma}
...
\begin{lemma}
    ...
    \begin{equation*}
        \psi\left (\sigma_-,y(\sigma_-)\right) >\frac{1}{M}, \quad \psi\left (\sigma_+,y(\sigma_+)\right) \leq\frac{\eta}{M}.
    \end{equation*}
\end{lemma}
\proof{Proof.}
First, we show that $\psi(\sigma_+,y(\sigma_+))\leq\frac{\eta}{M}$,
\begin{equation*}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation*}
The second line is from $\nabla^2 f(y(\sigma_+))\succeq 0$, \eqref{eq.bounded gradient y} and \eqref{eq.bracket points}. For the lower endpoint, failure of the early-termination test at line~\ref{line.early termination criterion} gives $\lambda_- > (\theta-1)\sigma_-$, so \Cref{lem.reduce line search} yields
\(
    \psi(\sigma_-,y(\sigma_-))>1/M.
\)
\Halmos \endproof
...
\begin{gather}
    ...
    \psi_+:=\psi(\sigma_+,y(\sigma_+))\leq\frac{\eta}{M}, \quad \psi_-:=\psi(\sigma_-,y(\sigma_-))>\frac{1}{M},\\
...
To derive the final oracle complexity result of \Cref{alg.ms accelerated utr}, one final step remains: we must get rid of the boundedness assumption used in \eqref{eq.bounded gradient y} and \eqref{eq.bounded assm point main text}. We establish this in \Cref{lem.bounded iterate}, whose proof can be found in \Cref{sec.appendix 2}.
\begin{lemma}
    \label{lem.bounded iterate}
    Suppose Assumption~\ref{assm.lipschitz} and Assumption~\ref{assm.solvable} hold, let $G_0>0$ satisfy \eqref{eq.certified G0}. For every $k\geq0$ for which the outer state $(x_k,v_k)$ is generated,
    \begin{equation}
    ...
    \end{gather}
\end{lemma}
By this lemma and the certified-bound condition \eqref{eq.certified G0}, we know \eqref{eq.bounded gradient y} holds, and \eqref{eq.bounded assm point main text} holds at every generated outer state with $M_0 =c_\gamma D_0$. Now we finally arrive at the final theorem of \Cref{alg.ms accelerated utr}, as a consequence of \Cref{thm.outer loop complexity}, \Cref{lem.complexity bisection}, and \Cref{lem.bounded iterate}.
\begin{theorem}
...
\begin{figure}[!t]
    ...
    \subcaptionbox{\texttt{a4a}, $N=4781, n=122$, nnz: 11.36\%.}[0.42\columnwidth]{
        \resizebox{0.42\columnwidth}{!}{\input{../figs/e-logistic-a4a-k}}
    }
    \subcaptionbox{\texttt{a9a}, $N=32561, n=123$, nnz: 11.27\%.}[0.42\columnwidth]{
        \resizebox{0.42\columnwidth}{!}{\input{../figs/e-logistic-a9a-k}}
    }
    \subcaptionbox{\texttt{w4a}, $N=7366, n=300$, nnz: 3.89\%.}[0.42\columnwidth]{
        \resizebox{0.42\columnwidth}{!}{\input{../figs/e-logistic-w4a-k}}
    }
    \subcaptionbox{\texttt{w8a}, $N=49749, n=300$, nnz: 3.88\%.}[0.42\columnwidth]{
        \resizebox{0.42\columnwidth}{!}{\input{../figs/e-logistic-w8a-k}}
    }
...
Secondly, in the beginning of the iterations, all accelerated methods, including \atr{}, \atrms{}, and \arca{}, converge faster than the non-accelerated methods (e.g., \utr{}~\texttt{(1)}, \utr{}~\texttt{(2)}, \arc{}). This confirms the effectiveness of the global acceleration. Thirdly, the global convergence does come at the expense of local superlinear convergence.
...
\begin{APPENDICES}
    ...
    \begin{lemma}
        ...
        For any $y\in \mathbb{R}^n$, let $H:=\nabla^2 f(y)\succeq0$ and $g:=\nabla f(y)$, and write
        \[
            H=Q\operatorname{diag}(h_1,\ldots,h_n)Q^\top,
            \qquad Q=[q_1,\ldots,q_n],
        \]
        where $Q$ is orthogonal. Define $\beta_i:=q_i^\top g$. Then $g_y(\mu)$ is continuously differentiable and monotonically increasing for $\mu \in \left(0,+\infty\right)$ and
        \begin{equation}
            \label{eq.derivative}
            g_y'(\mu) = \frac{1}{\|\left(H+\mu I\right)^{-1}g\|}+\frac{\mu}{\|\left(H+\mu I\right)^{-1}g\|^3}\sum_{i=1}^n\frac{\beta_i^2}{(h_i+\mu)^3},
        \end{equation}
        and it is bounded both below and above
        \begin{equation}
            \label{eq.derivative bound}
            0< g_y'(\mu) \leq \frac{2}{\|\left(H+\mu I\right)^{-1}g\|}.
        \end{equation}
    \end{lemma}
    \proof{Proof.}
    Using the spectral decomposition of $H$ above, for $\mu>0$ we have
    \begin{equation*}
        \left\|(H+\mu I)^{-1}g\right\|=\sqrt{\sum_{i=1}^n \frac{\beta_i^2}{(h_i+\mu)^2}}, \qquad g_y(\mu) = \frac{\mu}{\sqrt{\sum_{i=1}^n \frac{\beta_i^2}{(h_i+\mu)^2}}}.
    \end{equation*}
    Clearly, $g_y(\mu)$ is increasing on $\left(0,+\infty\right)$. Direct differentiation gives
    \begin{equation*}
        \begin{aligned}
            g_y'(\mu) & = \frac{1}{\sqrt{\sum_{i=1}^n \frac{\beta_i^2}{(h_i+\mu)^2}}}+\mu \frac{1}{\left(\sum_{i=1}^n \frac{\beta_i^2}{(h_i+\mu)^2}\right)^{3/2}}\sum_{i=1}^n\frac{\beta_i^2}{(h_i+\mu)^3} \\
                      & = \frac{1}{\|(H+\mu I)^{-1}g\|}+\frac{\mu}{\|(H+\mu I)^{-1}g\|^3}\sum_{i=1}^n\frac{\beta_i^2}{(h_i+\mu)^3} \\
                      & \leq_{(a)} \frac{1}{\|(H+\mu I)^{-1}g\|}+\frac{\mu}{\|(H+\mu I)^{-1}g\|^3}\sum_{i=1}^n\frac{\beta_i^2}{\mu(h_i+\mu)^2} \\
                      & = \frac{2}{\|(H+\mu I)^{-1}g\|}.
        \end{aligned}
    \end{equation*}
    $(a)$ follows from $h_i\geq0$ for $i=1,\ldots,n$.
    \Halmos \endproof
    ...
    {\color{red}
        ...
        We first lower bound the length \(r_u-r_l\). Let \(H:=\nabla^2 f(y)\),
        \(g:=\nabla f(y)\), and write
        \[
            H=Q\operatorname{diag}(h_1,\ldots,h_n)Q^\top,
            \qquad
            Q=[q_1,\ldots,q_n],
        \]
        where $Q$ is orthogonal, and set \(\beta_i:=q_i^\top g\). By convexity and
        Assumption~\ref{assm.bounded hessian}, \(0\le h_i\le \kappa_H\) for
        \(i=1,\ldots,n\). For \(\mu>0\),
        \(r_y(\mu)=\left(\sum_{i=1}^n\beta_i^2/(h_i+\mu)^2\right)^{1/2}\).
        Therefore,
        \begin{equation}
        ...
        \end{equation}
        Since \(\xi\le \mu_u\) and \(h_i\le \kappa_H\), we have
        \(1/(h_i+\xi)\ge 1/(\kappa_H+\mu_u)\). Hence,
        \begin{equation}
            \sum_{i=1}^n
            \frac{\beta_i^2}{(h_i+\xi)^3}
            =
            \sum_{i=1}^n
            \frac{\beta_i^2}{(h_i+\xi)^2}
            \frac{1}{h_i+\xi}
            \ge
            \frac{1}{\kappa_H+\mu_u}
            \sum_{i=1}^n
        ...
        \(r_u-r_l\ge r_y(\xi)(\mu_u-\mu_l)/(\kappa_H+\mu_u)\).
        Since \(r_y(\mu)\) is decreasing and \(\xi\le \mu_u\), we have
        \(r_y(\xi)\ge r_y(\mu_u)\). Moreover, \Cref{lem.bound target interval} gives
        \(\mu_u-\mu_l\ge M r_y(\mu_u)/2\). Therefore,
        \begin{equation}
        ...
        \end{equation}
        Finally, since \(0\preceq H\preceq \kappa_H I\), we have
        \(r_y(\mu_u)=\|(H+\mu_u I)^{-1}g\|\ge \|g\|/(\kappa_H+\mu_u)\).
        ...
        \(r_+=\|(H+M\|d_+\|I)^{-1}g\|\). Since \(H\succeq0\), it follows that
        \(r_+\le \|g\|/(M\|d_+\|)\).
        ...
        of \Cref{alg.local detection} gives \(\mu_+/\|d_+\|>2M\). By the definition
        of \(\mu_+\), \(\|g\|=2\mu_+^2/M\), so \(\|g\|>8M\|d_+\|^2\).
        Consequently, \(M^2\|d_+\|\|g\|>8M^3\|d_+\|^3\).
        ...
        Thus \(\|d_+\|>\min\{\epsilon_g/(2\kappa_H),
        \sqrt{\epsilon_g/M}\}\). Consequently,
        \[
            \frac{1}{\|d_+\|^3}
            \le
            \max\left\{
    ...
    {\color{red}
        ...
        $x$ with $\|x-x^*\|\geq 2\nu/(3M)$. Hence,
        \(f(x_k)-f^*<2\nu^3/(27M^2)\) implies $x_k\in\mathcal{LQ}_p$.
        In particular, the preceding global bound shows that this holds whenever
        \(k\geq 648^{1/3}MR/\nu\).
        ...
        \[
            ...
            \leq \frac{48MR^3}{k^3}.
        \]
        Thus \(\|x_k-x^*\|\leq \sqrt{288MR^3/(\nu k^3)}\).

        Using the definition
        \(y_k=\frac{k}{k+3}x_k+\frac{3}{k+3}v_k\), we obtain
        \[
            \|y_k-x^*\|
        ...
        Finally, Assumptions~\ref{assm.bounded hessian} and
        \ref{assm.local strongly convex} imply $\kappa_H\geq \nu$, so the second
        term in the definition of \(\rho\) is smaller and
        \(\rho=2\nu^2/(9\kappa_HM)\). If
        \(k\geq 36\kappa_H M R/\nu^2\), then this condition also implies
        \(k\geq 648^{1/3}MR/\nu\),
    ...
    \end{equation*}
    Since \eqref{eq.relation lambda sigma} and $\sigma_i>0$ imply $\|d_i\|>0$, set
    \(
        \zeta_i:=\|d_i\|^{1/2}>0
    \)
    and
    \(
        D:=\frac{4}{3\gamma M}\|v_0-x^*\|^2.
    \)
    The preceding bound on $B_k$ gives
    \(
        \sum_{i=0}^{k-1}A_{i+1}\zeta_i^6\leq D.
    \)
    H\"older's inequality therefore yields
    \begin{equation*}
        \sum_{i=0}^{k-1}A_{i+1}^{1/7}
        =\sum_{i=0}^{k-1}
        \left(A_{i+1}\zeta_i^6\right)^{1/7}
        \left(\zeta_i^{-1}\right)^{6/7}
        \leq
        D^{1/7}
        \left(\sum_{i=0}^{k-1}\zeta_i^{-1}\right)^{6/7}.
    \end{equation*}
    Consequently,
    \begin{equation*}
        \sum_{i=0}^{k-1}\frac{1}{\|d_i\|^{1/2}}
        \geq
        \left(\frac{3\gamma M}{4\|v_0-x^*\|^2}\right)^{1/6}
        \left(\sum_{i=0}^{k-1}A_{i+1}^{1/7}\right)^{7/6}.
    \end{equation*}
    Combining this inequality with \eqref{eq.Ak helper 1} gives
    \begin{equation}
    ...
    \end{lemma}
    \proof{Proof.}
    Let $H:=\nabla^2 f(y)\succeq0$ and $g:=\nabla f(y)$. Since all matrices below are spectral functions of the same symmetric matrix $H$, write
    \(
        H=Q\operatorname{diag}(h_1,\ldots,h_n)Q^T
    \)
    with $h_j\geq0$. The componentwise inequalities
    \(
        (h_j+\Bar{\sigma})^{-1}\leq(h_j+\sigma)^{-1}
    \)
    imply
    \begin{equation*}
        \Bar{\sigma}\psi(\Bar{\sigma},y)
        =\|(H+\Bar{\sigma}I)^{-1}g\|
        \leq\|(H+\sigma I)^{-1}g\|
        =\sigma\psi(\sigma,y).
    \end{equation*}
    ...
    \(
        \sigma/(h_j+\sigma)\leq
        \Bar{\sigma}/(h_j+\Bar{\sigma})
    \), so simultaneous diagonalization gives
    \begin{equation*}
        \sigma^2\psi(\sigma,y)
        =\|(H/\sigma+I)^{-1}g\|
        \leq\|(H/\Bar{\sigma}+I)^{-1}g\|
        =\Bar{\sigma}^2\psi(\Bar{\sigma},y).
    \end{equation*}
    Rearranging these two inequalities proves \eqref{eq.psi property wrt sigma}.
    \Halmos \endproof
    \begin{lemma}
    ...
    \begin{lemma}[Lemma 7.11, \citet{monteiro2013accelerated}]
    ...
    \begin{equation*}
        \|y(s)-y(t)\| = \left|\tau'(\xi)\right|(s-t) \|v-x\|\leq 2M_0\left|\tau'(\xi)\right|(s-t),
    \end{equation*}
    ...
    If $A=0$, then $\tau(\sigma)\equiv1$, so $y(s)=y(t)$ and \eqref{eq.curve condition} follows immediately. Hence, assume $A>0$ below.
    For $A>0$ and all $\sigma>0$, we have
    \begin{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            \psi_- -\psi_+
             ...
             & \leq_{(a)} \left(\frac{\sigma_+}{\sigma_-}\right)^2 \psi(\sigma_+,y(\sigma_-)) - \psi(\sigma_+,y(\sigma_+)) \\
             & \leq_{(b)} \left(\frac{\sigma_+}{\sigma_-}\right)^2 \Bigg[
                \frac{1}{\sigma_+}\|y(\sigma_-)-y(\sigma_+)\|
        ...
        \end{aligned}
    \end{equation*}
    $(a)$ is because of \Cref{lem.psi property wrt sigma}, $(b)$ is because of \Cref{lem.psi property wrt y}. To derive \eqref{eq.bounded gap bisection sigma}, plug
    ...
    \begin{lemma}
        ...
        \eqref{eq.bounded assm point} holds. Suppose also that the same certified bound $G_0>0$ used by \textnormal{R\&B} satisfies
        \(
            \|\nabla f(y(\sigma))\|\leq G_0
        \)
        for every $\sigma>0$. Then in \textnormal{R\&B}, we have
        \begin{equation}
    ...
    \[
        ...
        \right).
    \]
    For any fixed certified input $G_0$ satisfying \eqref{eq.certified G0} and
    $M_0=c_\gamma D_0$, both $G_0$ and $M_0$ are independent of
    $\epsilon_g$. Therefore, for fixed
    $M,D_0,\|H_*\|,\theta,\eta$, and $\gamma$, we conclude
    \(
        N_k=O\!\left(\log(1/\epsilon_g)\right).
    \)
    \Halmos
    \endproof
    ...
    \begin{equation*}
        \begin{aligned}
            \|\nabla f(y_k(\sigma)) \| & \leq \|\nabla f(x^*)+\nabla^2 f(x^*)(y_k(\sigma)-x^*)\| +\frac{M}{2}\|y_k(\sigma)-x^*\|^2                                    \\
                                       & \leq \|\nabla^2 f(x^*)(y_k(\sigma)-x^*)\|+\frac{M}{2}\|y_k(\sigma)-x^*\|^2                                                   \\
                                       & \leq c_\gamma\|H_*\|D_0+\frac{M}{2}c_\gamma^2D_0^2\leq G_0.
        \end{aligned}
    \end{equation*}
    We use induction on the generated outer states, with the additional invariant that every preceding non-early-terminated R\&B call satisfies \eqref{eq.relation lambda sigma}. For $k=0$, $A_0=0$ and $x_0=v_0$ imply $y_0(\sigma)=v_0=x_0$ for every $\sigma>0$. Thus
    \(
        \|x_0-x^*\|=\|v_0-x^*\|=D_0\leq c_\gamma D_0,
    \)
    and the Taylor estimate above gives $\|\nabla f(y_0(\sigma))\|\leq G_0$.

    Suppose the invariant holds through a generated outer state $(x_k,v_k)$, and set $M_0:=c_\gamma D_0>0$. Since $y_k(\sigma)$ is a convex combination of $x_k$ and $v_k$, the induction bounds give
    \(
        \|y_k(\sigma)-x^*\|\leq c_\gamma D_0
    \)
    for every $\sigma>0$; the Taylor estimate above then gives $\|\nabla f(y_k(\sigma))\|\leq G_0$. Consequently, \Cref{lem.bracketing points,coro.bisection bound} guarantee that the $k$-th R\&B call terminates after finitely many oracle calls. If it terminates with ET=True, no outer state $(x_{k+1},v_{k+1})$ is generated and there is nothing further to prove. Otherwise, at loop exit case ii) is false, so $0\leq\lambda_k\leq(\theta-1)\sigma_k$. If $\lambda_k=0$, the negation of case i) gives $\|d_k\|\geq\eta\sigma_k/M$; if $\lambda_k>0$, trust-region complementarity gives $\|d_k\|=\sigma_k/M\geq\eta\sigma_k/M$. Hence \eqref{eq.relation lambda sigma} holds at iteration $k$, and therefore at every iteration $i=0,\ldots,k$ by the induction invariant.

    We may now apply \eqref{eq.ms estimate advanced}. Since $f(x_{k+1})-f^*\geq0$ and $B_{k+1}\geq0$, it gives
    \(
        \|v_{k+1}-x^*\|\leq D_0.
    \)
    It remains to bound $x_{k+1}$. Note that
    \begin{equation*}
        \begin{aligned}
            \|x_{k+1}-x^*\| & \leq \|x_{k+1}-y_k\| + \|y_k-x^*\|                                            \\
                            & \leq_{(a)} \|d_k\|+ \frac{A_k}{A_{k+1}} \|x_k-x^*\| +\frac{a_k}{A_{k+1}}\|v_k-x^*\| \\
                            & \leq_{(b)} \frac{1}{A_{k+1}}\left(A_{k+1}\|d_k\| +A_k\|x_k-x^*\| +  a_k D_0\right)  \\
                            & \leq_{(c)} \frac{1}{A_{k+1}}\left (\sum_{i=0}^k A_{i+1}\|d_i\|\right)+D_0.
        \end{aligned}
    \end{equation*}
    $(a)$ is from \eqref{eq.update yk}, $(b)$ uses the induction bound $\|v_k-x^*\|\leq D_0$, and $(c)$ follows by iterating the same inequality and using $\|v_i-x^*\|\leq D_0$ for $i=0,\ldots,k$:
    \begin{equation*}
        A_{k+1} \|x_{k+1}-x^*\|\leq A_{k+1}\|d_k\|+A_k\|x_k-x^*\|+a_k D_0,
    \end{equation*}
    This recurrence is obtained by multiplying the third line by $A_{k+1}$.

    Since \eqref{eq.relation lambda sigma} has now been established through iteration $k$, summing \eqref{eq.helper bounded x} is valid and gives
    \begin{equation*}
    ...
    \end{equation*}
    Together with the bound on $v_{k+1}$ obtained above, this proves \eqref{eq.bounded iterate} for the next generated outer state. Since $y_{k+1}(\sigma)$ is a convex combination of $x_{k+1}$ and $v_{k+1}$ and $c_\gamma\geq1$, we also have
    \(
        \|y_{k+1}(\sigma)-x^*\|\leq c_\gamma D_0
    \)
    for every $\sigma>0$. Applying the Taylor estimate at the start of the proof yields $\|\nabla f(y_{k+1}(\sigma))\|\leq G_0$, closing the induction.
    \Halmos \endproof
\end{APPENDICES}
