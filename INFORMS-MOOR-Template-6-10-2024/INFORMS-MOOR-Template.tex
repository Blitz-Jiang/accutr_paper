...
%%\DoubleSpacedXI
%%\DoubleSpacedXII
\input{INFORMS-MOOR-Template-6-10-2024/header_mor}
...
On the practical side, the TR oracle has been thoroughly analyzed \cite{jiangHolderianErrorBounds2022,rojasNewMatrixfreeAlgorithm2001,ho2017second,wang2022generalized,so2007approximating,so2011deterministic}, so that highly competitive solvers have been developed \cite{rojasNewMatrixfreeAlgorithm2001, adachiSolvingTrustregionSubproblem2017, adachiEigenvaluebasedAlgorithmAnalysis2019} for it and its extensions.
To this end, TR oracles have formed the algorithmic core of general-purpose nonlinear programming solvers such as Knitro~\cite{byrd2006k}, COPT~\cite{ge2022cardinal}, and PDFO~\cite{ragonneau2024pdfo}. Similar success can be found in other cross-cutting applications ranging from adversarial training~\cite{yao2019trust} to reinforcement learning~\cite{schulman2015trust}. These advantages serve as the motivation to use \tr{} to address the global-local balance in accelerated SOMs.
...
In this section, we propose the first accelerated TR-type method, which applies the primal-dual structure of \ref{eq.newTR} into the estimating sequence \cite{nesterov2008accelerating}. The global oracle complexity for finding $\epsilon_f$-approximate-solution is improved to $\tilde O(\epsilon_f^{-1/3})$ as opposed to the recent $O(\epsilon_f^{-1/2})$ in convex optimization \cite{jiang2024nonconvexityuniversaltrustregionmethod}, while the local rate of convergence to the non-degenerate solution remains quadratic.
...
\Halmos \endproof
To maintain the flow of the complexity analysis, we temporarily assume the output of R\&B of \Cref{alg.local detection} satisfies~\eqref{eq.output of alg2} as well, i.e., \eqref{eq.output of alg2} holds for $k\in\mathcal{Z}$~(we will rigorously verify this in Section~\ref{sec.line search}). Now we show that the relation \eqref{eq.es relation} holds during the update of \Cref{alg.accelerated utr}.
...
\begin{theorem}
    ...
    \[
        f(x_k)-f^*
        \leq
        \frac{48M\|x_0-x^*\|^3}{k(k+1)(k+2)}.
    \]
\end{theorem}
}
{\color{red} 
\proof{Proof.}
Let $R=\|x^*-x_0\|$; by \Cref{lem.estimate sequence} with $x=x^*$, for every $k\ge1$, $f(x_k)-f^*\le \phi_0(x^*)/A_k=8MR^3/(k(k+1)(k+2)/6)=48MR^3/[k(k+1)(k+2)]$. 
\Halmos \endproof
\begin{corollary}
    ...
    hold, and suppose that the ratio condition required by
    \Cref{lem.estimate sequence} is satisfied at every non-terminated
    iteration. Then it takes \Cref{alg.accelerated utr} $O\left(\left(\frac{MD_0^3}{\epsilon_f}\right)^{1/3}\right)$ iterations to find $\epsilon_f$-function-value solutions as in \Cref{def.approximate solution}.
\end{corollary}
}
\subsubsection{Oracle complexity of \Cref{alg.local detection}}\label{sec.line search}
...
\begin{corollary}
    ...
    Suppose Assumption~\ref{assm.lipschitz}, Assumption~\ref{assm.bounded hessian}, and the "if" condition $\|d_+\|\leq \min\{\frac{\epsilon_g}{2\kappa_H},\sqrt{\frac{\epsilon_g}{M}}\}$ at line~\ref{line.if coro 3.1} of \Cref{alg.local detection} holds, then we have
    \begin{equation*}
...
\proof{Proof.}
    ...
    \end{equation*}
    {\color{red} where the first in equality is also from $\lambda=0$ and Line~\ref{line.set-params}.}
\Halmos \endproof
...
Till now, we have established that the oracle complexity of \Cref{alg.local detection} reduces to $\max\{O\left(\log\frac{\kappa_H+\mu_+}{\epsilon_g}\right),O\left(\log \frac{\|\nabla f(y)\|}{\epsilon_g}\right)\}$. Since $\mu_+ = \frac{\sqrt{2M}}{2}\|\nabla f(y)\|^{1/2}$ and we already established the boundedness for the gradient norm in \Cref{lem.upper bound for zero lambda}, we can derive the total complexity of \Cref{alg.accelerated utr},
\begin{theorem}
    ...
    \end{equation}
    \ref{eq.newTR} oracles for \Cref{alg.accelerated utr} to find an $(\epsilon_f,\epsilon_g)$-approximate solution that satisfies \Cref{def.approximate solution}.
\end{theorem}
\proof{Proof.}
...
and \ref{assm.local strongly convex} hold. Let
$R=\|x_0-x^*\|$. Then, whenever the $k$-th iterate of
...
We now turn to another accelerated TR method that incorporates the acceleration framework of \citet{monteiro2013accelerated}. This method achieves a near-optimal global oracle complexity of $\tilde{O}(\epsilon_f^{-2/7})$ for finding $(\epsilon_f,\epsilon_g)$-approximate solutions. However, this global efficiency comes at the cost of losing local quadratic convergence. We refer to this approach as the Accelerated Trust-Region Extragradient Method (\Cref{alg.ms accelerated utr}).
\subsection{Algorithm design}
...
At the core of \Cref{alg.ms accelerated utr} lies an implicit search procedure (\Cref{alg.bisection ms}, we still call it R\&B for short) along the curve {\color{red} probably replace $y_k$ with $y_k(\sigma)$}:
...
\begin{subroutine}[!ht]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE{Set $\sigma_- = \sqrt{\frac{2M\epsilon_g}{1+2\theta}}, \ \sigma_+=\sqrt{\frac{MG_0}{\eta}}$, $(\lambda_-,d_-) = \trp(y(\sigma_-),\sigma_-,\frac{1}{M}\sigma_-)$
            \label{line.def G0}
        }
        ...
        \STATE{$(\lambda,d) = \trp(y(\sigma),\sigma,\frac{1}{M}\sigma)$\label{line.call-utr-bisec}}
        \IF{i) holds}
        \STATE{$\sigma_+ =\sigma$}
        \ELSIF{ii) holds}
        \STATE{$\sigma_- =\sigma$}
...
\begin{lemma}
    ...
    \begin{equation*}
        \|\nabla f(y(\sigma_-)+d_-) \|\leq \epsilon_g.
    \end{equation*}
\end{lemma}
\proof{Proof.}
    From \Cref{lem.lipschitz}, we have
    \begin{equation*}
...
\proof{Proof.}
    ...
    The third line comes from \eqref{eq.update yk}. The last line is from the convexity of $f$. {\color{red} Using $A_{k+1}=A_k+a_k$ as in \Cref{alg.ms accelerated utr}} and rearranging terms gives
    \begin{align*}
    ...
    \end{align*}
    The second inequality is from {\color{red} whenever we call the oracle at Line~\ref{line.def G0} and \ref{line.call-utr-bisec}, we have $\sigma \geq M\|d\|$, hence we can apply \eqref{eq.inner product g d}}. The third inequality is from \eqref{eq.relation lambda sigma}. {\color{red} Note that $\gamma \leq \frac{1}{\theta}$ and \eqref{eq.update ak} hold. We have 
    \begin{equation*}
...
\begin{lemma}
    ...
    \begin{equation}
        ...
        A_k \geq \left(\frac{\eta}{4}\left (\frac{3\gamma}{4M^2D_0^2}\right)^{1/3}\right)^{3 / 2}\left(\frac{2 k+1}{3}\right)^{7/2} = \Omega(\frac{k^{7/2}}{MD_0}).
    \end{equation}
\end{lemma}
...
\begin{theorem}
{\color{red}[Theorem 4.3.2, \citet{nesterov_lectures_2018}]}\label{thm.outer loop complexity}
    Suppose Assumption~\ref{assm.lipschitz} and Assumption~\ref{assm.solvable}
    hold, and suppose that \eqref{eq.relation lambda sigma} is satisfied at every non-terminated
    iteration. Then it takes \Cref{alg.ms accelerated utr} $O(MD_0^3\epsilon_f^{-2/7})$ iterations to find $\epsilon_f$-function-value solutions as in \Cref{def.approximate solution}.
\end{theorem}
...
As mentioned, this problem is a notorious example where many accelerated SOMs are not as competitive as the classical Newton-type methods \cite{carmon2022optimal,chen2022accelerating,zhang2012local} numerically.
...
\proof{Proof.}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    The inequality is because of $\zeta_i \geq 0$ for $i=1,\ldots,n$.
\Halmos \endproof

Now we are ready to formally prove \Cref{lem.bound target interval}.
\proof{Proof.}
{\color{red} seems requires $M>1$}
...
\proof{Proof.}
    ...
    \end{equation*}
    For $r_y(\mu_l)-r_y(\mu_u)$, using mean value theorem, there exists $\xi\in[\mu_l,\mu_u]$ such that {\color{red} check the second and third line, elaborate}
    \begin{equation*}
        \begin{aligned}
            r_y(\mu_l)-r_y(\mu_u) & =r_y'(\xi)(\mu_l-\mu_u)                                                                             \\
                                  & =_{{\color{red} \Cref{lem.property g}}}\frac{\sum_{i=1}^n \frac{\beta_i^2}{(\zeta_i+\xi)^3}}{\|(\nabla^2 f(y)+\xi I)^{-1}\nabla f(y)\|}(\mu_u-\mu_l) \\
                                  & \geq \frac{1}{\kappa_H+\mu_u} \|(\nabla^2 f(y)+\xi I)^{-1}\nabla f(y)\|(\mu_u-\mu_l)                             \\
                                  & \geq \frac{1}{\kappa_H+\mu_u} \|(\nabla^2 f(y)+\mu_u I)^{-1}\nabla f(y)\|^2                                      \\
    ...
    \begin{equation*}
        \begin{aligned}
            O\left(\log\frac{r_+}{r_y(\mu_l)-r_y(\mu_u)}\right) & \leq O\left(\log\frac{r_+(\kappa_H+\mu_u)^3}{\|\nabla f(y)\|^2}\right)                                                                                \\
                                                                ...
                                                                & \leq_{{\color{red} ck}} O\left(\log\frac{(\kappa_H+\mu_+)^3}{8M^2\|d_+\|^3}\right)                                                                                         \\
                                                                & \leq_{{\color{red} ad}}  \max \left\{O\left(\log\frac{(\kappa_H+\mu_+)^3}{8\epsilon_g}\right),O\left(\log\frac{4\kappa_H^2(\kappa_H+\mu_+)^3}{8M\epsilon_g^2}\right)\right\} \\
                                                                & = O\left(\log\frac{\kappa_H+\mu_+}{\epsilon_g}\right).
        \end{aligned}
    \end{equation*}
    In the above, the second line is from 
    {\color{red} that the staring value as in Line~\ref{line.left bracketing point} of \Cref{alg.local detection}.}
    $$r_+ = \|(\nabla^2 f(y)+M\|d_+\|I)^{-1} \nabla f(y)\| \leq \frac{\|\nabla f(y)\|}{M\|d_+\|}.$$
    The fourth line is from $\mu_u \leq \mu_+$ and {\color{red} from Line~\ref{line.early terminate main} in \Cref{alg.accelerated utr}}
    $$
        \frac{\sqrt{2M}}{2}\|\nabla f(y)\|^{1/2} =\mu_+ \geq 2M\|d_+\|.
    $$
...
%     \end{equation*}
%     Since $\frac{1}{2\kappa_H}\|\nabla f(x_k)\|^2 \leq f(x_k)-f^* \leq \frac{3M\|x_0-x^*\|^3}{4 k(k+1)(k+2)}$~({\color{red} the LHS of the inequality is from \citet[Lemma 1.2.2, Theorem 2.1.5]{nesterov_lectures_2018}}), we have
%     \begin{equation*}
...
\proof{Proof.}
    ...
    \begin{equation*}
        \frac{1}{\zeta_i^2}={\color{red} 6?}w A_{i+1} \zeta_i^5, \quad i=0, \ldots, k-1
    \end{equation*}
...
\proof{Proof.}
    ...
    \end{equation*}
    {\color{red} also review the constant term in the statement
    ...
    \begin{equation*}
            \left|\tau'(\sigma)\right| \leq \frac{2A}{\left(1+\sqrt{1+4A\sigma}\right)^2}  \leq \frac{2A}{4A\sigma}  = \frac{1}{2\sigma}.
    \end{equation*}
    Therefore we have
    \begin{equation*}
        \begin{aligned}
...
\begin{lemma}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            \right]                                                                                                                                                                                                                                          \\
             & \leq \sqrt{G_0}\left(\frac{(1+2\theta)^{3/2}M_0}{2\sqrt{2}M\eta \epsilon_g^{3/2}}+\frac{(1+2\theta)^2M_0^2}{4\sqrt{M}\eta\epsilon_g^2}+\frac{(1+2\theta)^{3/2}M_0}{\sqrt{2}M\epsilon_g^{3/2}}+\frac{(1+2\theta)\sqrt{G_0}}{M^{3/2}\epsilon_g}\right).
        \end{aligned}
    \end{equation*}
    Denote
    $$C(G_0,M_0,M,\epsilon_g):= M\sqrt{G_0}\left(\frac{(1+2\theta)^{3/2}M_0}{2\sqrt{2}M\eta \epsilon_g^{3/2}}+\frac{(1+2\theta)^2M_0^2}{4\sqrt{M}\eta\epsilon_g^2}+\frac{(1+2\theta)^{3/2}M_0}{\sqrt{2}M\epsilon_g^{3/2}}+\frac{(1+2\theta)\sqrt{G_0}}{M^{3/2}\epsilon_g}\right),$$
    we have
    \begin{equation}
        \label{eq.bound value by brackets}
        \sigma_+-\sigma_->\frac{1-\eta}{C(G_0,M_0,M,\epsilon_g)}.
    \end{equation}
    Further, the oracle called during the bisection is bounded by
    \begin{equation*}
        \log \left(\frac{\sqrt{\frac{MG_0}{\eta}}C(G_0,M_0,M,\epsilon_g)}{1-\eta}\right).
    \end{equation*}
\end{lemma}
\proof{Proof.}
    Note that if the bisection procedure does not terminate, from \eqref{eq.bound value by brackets} we have
    \begin{equation*}
        \sigma_+-\sigma- > \frac{1-\eta}{ C(G_0,M_0,M,\epsilon_g)}.
    \end{equation*}
    ...
    \begin{equation*}
            N_k \leq \log \left(\frac{\left(\sqrt{\frac{MG_0}{\eta}}-\sqrt{\frac{2M\epsilon_g}{1+2\theta}}\right)C(G_0,M_0,M,\epsilon_g)}{1-\eta}\right) \leq \log \left(\frac{\sqrt{\frac{MG_0}{\eta}}C(G_0,M_0,M,\epsilon_g)}{1-\eta}\right).
    \end{equation*}
    Since $G_0,M_0$ is polynomial in $D_0$, by the definition of $C(G_0,M_0,M,\epsilon_g)$ and omitting the algorithm parameters, we conclude $N_k \leq O\left(\log MD_0/\epsilon_g\right)$.
\Halmos \endproof
