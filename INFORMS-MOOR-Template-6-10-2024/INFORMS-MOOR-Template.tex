...
For that sake, one particular choice could be the trust-region (TR) oracle~\cite{more1983recent,conn2000trust}. At a given point \( x \in \mathbb{R}^n \), the classic TR oracle solves the following subproblem:
\begin{equation}
...
Moreover, \tr{} oracles can be solved in $O(n^3 + n^2\log\log(\epsilon^{-1}))$ arithmetic operations \cite{vavasis1990proving,ye1991new}, incurring no significant overhead compared to solving a Newton equation.
...
\begin{theorem}
    {\color{red}[Theorem 4.3.2, \citet{nesterov2018lectures}]}\label{thm.outer loop complexity}
    Suppose Assumption~\ref{assm.lipschitz} and Assumption~\ref{assm.solvable}
