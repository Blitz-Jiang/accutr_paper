...
\begin{corollary}
    ...
    iteration. Then it takes \Cref{alg.accelerated utr} \cz{$O\left(\frac{M^{1/3}D_0}{\epsilon_f^{1/3}}\right)$} iterations ($D_0 = \|x_0 - x^*\|$) to find $\epsilon_f$-function-value solutions as in \Cref{def.approximate solution}.
\end{corollary}
}
\begin{comment}
...
\cz{We now estimate the number of oracle calls in \Cref{alg.local detection}.
    ...
    In this part of the analysis, we may assume that $\|d_+\| > \min\{\frac{\epsilon_g}{2\kappa_H},\sqrt{\frac{\epsilon_g}{M}}\}$; otherwise, it ends at {Track 0}. Furthermore, since we also limit the calls in {Track~1}, we only have to investigate the oracle complexity of Track 2 (R\&B), i.e., the complexity of bisection over radius $r$ on the interval $[r_-,r_+]$. % to make \eqref{eq.output of alg2} hold.
...
\cz{
    ...
    \end{lemma}
    The proof is immediate since if ET=False, we have successfully located the auxiliary function such that
    \begin{equation*}
        M \leq g_y(\mu) \leq 2M.
    \end{equation*}
    Matching the condition needed by \Cref{thm.convergence outer loop}.}%
The following lemma validates the choices of the bracketing points at line~\ref{line.left bracketing point}.
\begin{lemma}
    \label{lem.valid bracketing}
    In \textnormal{R\&B} of \Cref{alg.local detection}, when the bisection begins, we have
...
% By the fact that oracle complexity of \Cref{alg.local detection} reduces to $\max\{O\left(\log\frac{\kappa_H+\mu_+}{\epsilon_g}\right),O\left(\log \frac{\|\nabla f(y)\|}{\epsilon_g}\right)\}$.
\cz{
    We begin with the following property of the iterates $y_k$ with $k\in\mathcal{Z}$, which invoked \Cref{alg.local detection}, cf. \eqref{eq.outer index catagory}.}
\begin{lemma}
    \label{lem.upper bound for zero lambda}
    Suppose Assumption~\ref{assm.bounded hessian} hold, for $k\in \mathcal{Z}$, we have
    \begin{equation}
