...
\title[Accelerating Trust-Region Methods]{Balancing Global and Local Efficiency by Accelerating Trust-Region Methods}
...
\author{Chuwen Zhang}
\address{Booth School of Business, University of Chicago, Chicago, 60637, IL, USA}
% \curraddr{}
...
In this context, two major approaches have emerged. The accelerated cubic regularized Newton (CRN) method~\cite{nesterov2008accelerating}, which builds on the cubic regularization oracle~\cite{nesterov2006cubic}, achieves a global oracle complexity of $O(\epsilon^{-1/3})$\footnote{Oracle complexity refers to the number of second-order subproblems (the oracle calls) needed to find an approximate solution.}. The accelerated Newton proximal extragradient (A-NPE) method~\cite{monteiro2013accelerated} uses an approximate Newton oracle and improves the oracle complexity to $\tilde{O}(\epsilon^{-2/7})$.
...
To begin with, let us introduce a unifying view of the updates of the second-order methods. In fact, almost all of them choose the step at some given point $x$ as follows:
\begin{equation}
    \label{eq.second-order update}
    \left (\nabla^2 f(x) + \mu I \right )d = -\nabla f(x).
\end{equation}
The regularization parameter $\mu$ plays a critical role in the convergence analysis and varies among different SOMs.
In the classical TR method using the standard oracle ~\eqref{eq.classic tr subproblem}, $\mu$ is an \emph{a posteriori} parameter --- the Lagrangian multiplier to the ball constraint.
In the regularized Newton methods, $\mu$ can be selected directly as $\Theta(\sqrt{\|\nabla f(x)\|})$ such that a $O(\epsilon^{-1/2})$ method can be designed \cite{mishchenko_regularized_2023}. For accelerated methods, both the CRN method~\cite{nesterov2008accelerating} and the A-NPE method~\cite{monteiro2013accelerated} require some search procedures on $\mu$. Nevertheless, the following lemmas clarify the role of $\mu$ for second-order methods.
\begin{lemma}
...
\begin{proof}
    ...
    \end{equation*}
    Applying the triangle inequality and rearrange items, we can derive \eqref{eq.bound next gradient}.
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{proof}
    \cz{This proof seems to be overlapping with the one above?}
...
\Cref{lem.inner product g and d} serves as a key bridge to establish the global efficiency --- $\mu$ should be selected roughly as $\Theta(\|d\|)$. This condition is maintained by CRN and A-NPE but generally fails for the classical trust-region oracle. For that reason, we turn to the $\trp{}$ oracle introduced in the previous section:
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
We denote $(d,\lambda) = \trp(x,\sigma,r)$ as the primal-dual solution of this problem. The global optimality conditions are as follows.
\begin{lemma}[Section 3,~\citet{conn2000trust}]\label{lem.optimal condition}
    ...
    \end{subequations}
\end{lemma}
In the $\trp{}$ oracle, the regularization parameter decomposes as $\mu=\sigma+\lambda$, aggregating primal \emph{a priori} and dual \emph{a posteriori} components. As we will show in the following, this primal-dual combo of information has the potential to balance global and local behavior.
The central task is, while preserving the local capabilities, to ensure \Cref{lem.inner product g and d} so that the method remains compatible with acceleration techniques.

% \cz{This lemma may better be claimed to $\mu$?}


\section{Variant~I: Resolving the Global-Local Trade-off}\label{sec.first variant}
In this section, we propose the accelerated TR method, which applies the primal-dual structure of $\trp$ into the estimating sequence \cite{nesterov2008accelerating}. The global oracle complexity of this TR variant is improved to $\tilde O(\epsilon^{-1/3})$ as opposed to the recent $\tilde O(\epsilon^{-1/2})$ in convex optimization \cite{jiang2024nonconvexityuniversaltrustregionmethod}, while the local rate of convergence to the non-degenerate solution remains quadratic. To our knowledge, this is also the first time that an accelerated TR method is designed.
...
\begin{subroutine}[!ht]
    ...
    \end{algorithmic}
\end{subroutine}
As we mentioned previously, the overall regularization in $\trp{}$ is a combination of primal and dual information: $\mu_k = \sigma_k + \lambda_k$.
% On the one hand, we carefully select $(\sigma_k, r_k)$ to match the conditions in \Cref{lem.inner product g and d}.
The algorithm proceeds by checking whether the current multiplier $\lambda_k$ is zero.

When $\lambda_k > 0$ (or sufficiently bounded away from zero), it typically indicates that the current iterate is still far from a local optimum (at least in view of the gradient norm). In that regime, as shown later in \Cref{lem.ratio pass nonzero}, the overall regularization parameter $\mu_k$ becomes automatically proportional to the step size. This further justifies the updates of the estimating sequence used in the acceleration framework.
...
Now we analyze the worst-case oracle complexity of \Cref{alg.accelerated utr} for computing an approximate solution as defined in \Cref{def.approximate solution}.

\subsubsection{Iterations complexity of \Cref{alg.accelerated utr}}
Since our goal here is to analyze the worst-case global complexity, we assume the iterates do not step into \texttt{Track 1} of \Cref{alg.local detection} does not occur, in which case it is mostly relevant to local convergence analysis (cf. \Cref{sec.local}). Denote $K_\epsilon$ as the iteration number \Cref{alg.accelerated utr} takes to find an approximate solution satisfying \eqref{eq.approximate solution}. We next show $K_\epsilon = O(\epsilon^{-1/3})$.
...
\end{proof}
To proceed, we categorize the iterates generated by \Cref{alg.accelerated utr} into two disjoint sets based on the value of the multiplier, according to the iterations with and without local detection:
\begin{equation}
...
In this section, we implement our accelerated Trust-Region method (\atr{}) and set comparisons to a few existing SOMs:
\begin{itemize}[leftmargin=*]
    \item The cubic regularization method (\arc{}, \citet{nesterov2006cubic}) and its accelerated version (\arca{}, \citet{nesterov2008accelerating}).
    \item A non-accelerated trust-region method using the $\trp{}$ oracle, by setting both $(\sigma_k, r_k)$ proportionally to $\|\nabla f(x_k)\|^{1/2}$. This method is termed \utr{} in a previous work \cite{jiang2024nonconvexityuniversaltrustregionmethod} targeting convex optimization.
    \item Two variants of accelerated trust-region method using the $\trp{}$ oracle: \atr{} and \atrms{}.
\end{itemize}

We conduct experiments on the regularized logistic regression problem, in which, numerically, many accelerated SOMs are not as competitive as the classical Newton-type methods \cite{carmon2022optimal,jiang2024accelerated,jiang2020unified}.
All experiments are conducted on a single machine with a 14-core Apple M4 Pro CPU  and 48GB LPDDR5 RAM.
The regularized Logistic regression problem is defined as follows.
\begin{equation}
...
This bound is quite conservative according to \cite{songUnifiedAccelerationHighorder2021}. However, we use it for a fair comparison and eliminate the effect from adaptive adjustments common in the literature. We test two non-accelerated trust-region methods $\utr{}~\texttt{(1)}, \utr{}~\texttt{(2)}$, using different Lipschitz estimates $\tfrac{\widehat M}{2}, \widehat M$, respectively; to present the sensitivity of Lipschitz constants and set a fair comparison to \arc{}. In \Cref{fig.logistic regression}, the performance of the SOMs on the LIBSVM datasets\footnote{For details, see \url{https://www.csie.ntu.edu.tw/cjlin/libsvmtools/datasets/}} are reported.
\begin{figure}[h!]
    \small
    \centering
...
We could have several observations. Firstly, for non-accelerated methods, a trend of local superlinear convergence can be observed in \utr{}~\texttt{(1)}, \utr{}~\texttt{(2)} and \arc{}. We could conclude that \utr{} and \arc{} are comparable.
Secondly, in the beginning of the iterations, all accelerated methods, including \atr{}, \atrms{}, and \arca{}, converge faster than the non-accelerated methods (e.g., \utr{}~\texttt{(1)}, \utr{}~\texttt{(2)}, \arc{}). This confirms the effectiveness of the acceleration techniques. However, only \atr{} has the local rate of superlinear convergence. Both \atrms{} and \arca{} converge sublinearly in the local regime, in which \atrms{} is slightly better because of its superior non-asymptotic performance.
