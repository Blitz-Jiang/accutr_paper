...
\author{Chuwen Zhang}
\address{Booth School of Business, University of Chicago, Chicago 60637, USA}
% \curraddr{}
...
\begin{abstract}
    Although accelerated second-order methods achieve improved global complexity in theory, they often underperform in high-accuracy regimes where rapid local convergence is crucial, as reported in, e.g.,~\cite{carmon2022optimal,chen2022accelerating,jiang2020unified}. We study whether second-order methods can be accelerated without sacrificing their local efficiency. Focusing on trust-region-type methods, whose Lagrange multipliers reveal local geometric information and thus enable strong local behavior, we develop the first accelerated trust-region-type methods that explicitly address the global-local trade-off. Our primary contribution is the \emph{Global-Local Accelerated Trust-Region Method}, which leverages the Lagrange multiplier to detect local regions and achieves $\tilde{O}(\epsilon^{-1/3})$ global oracle complexity while preserving quadratic local convergence. We also propose the \emph{Accelerated Trust-Region Extragradient Method}, which pushes global oracle complexity to the near-optimal rate of $\tilde{O}(\epsilon^{-2/7})$, though at the cost of weaker local performance. Together, these results offer a constructive solution to the global-local acceleration dilemma and establish the accelerated trust-region methods for the first time. Numerical experiments further demonstrate the practical advantages of our methods over existing accelerated and classical second-order algorithms.
\end{abstract}
\maketitle
...
\Cref{lem.inner product g and d} serves as a key bridge to establish the global efficiency --- $\mu$ should be selected roughly as $\Theta(\|d\|)$. This condition is maintained by CRN and A-NPE but generally fails for the classical trust-region oracle. For that reason, we turn to the $\trp{}$ oracle:
...
\begin{lemma}[Section 3,~\citet{conn2000trust}]\label{lem.optimal condition}
    The direction $d$ is the solution to \eqref{eq.newTR} if and only if there exists a dual multiplier $\lambda \geq 0$ such that: \begin{subequations}\label{eq.sa}
        \begin{align}
...
In this section, we propose the first accelerated TR method, which applies the primal-dual structure of $\trp$ into the estimating sequence \cite{nesterov2008accelerating}. The global oracle complexity is improved to $\tilde O(\epsilon^{-1/3})$ as opposed to the recent $\tilde O(\epsilon^{-1/2})$ in convex optimization \cite{jiang2024nonconvexityuniversaltrustregionmethod}, while the local rate of convergence to the non-degenerate solution remains quadratic.
...
By contrast, when $\lambda_k = 0$, a signal is provided that the current extrapolation point $y_k$ may have entered the local quadratic convergence region of Newton's method (see \Cref{lem.detection of local quadratic}). This triggers the local detection procedure (\Cref{alg.local detection}), which consists of two separate tracks probing the local geometry around $y_k$. In \texttt{Track 1} (local exploitation), we run Newton's method starting from $y_k$. As discussed in Section~\ref{sec.local}, if $y_k$ indeed lies within the quadratic convergence region, the resulting sequence will exhibit quadratic convergence, which justifies early termination of \Cref{alg.accelerated utr}. We remark that if we encounter degenerate Hessians in \texttt{Track 1}, we terminate it and continue with \texttt{Track 2} since this means we are not near a local optimizer. In \texttt{Track 2} (ratio bracketing), we attempt to find a step $d_k$ whose size is proportional to the regularization parameter $\mu_k$, we remark that in the bisection procedure, we set $r_+=r$ if $\frac{\mu}{\|d\|}<M$ and otherwise $r_-=r$. This step is easy to compute and always succeeds, ensuring that we can continue updating the estimate sequence and preserve global acceleration. In summary, \Cref{alg.accelerated utr} admits two possible termination routes: when $y_k$ enters the region of local quadratic convergence, the algorithm detects this and terminates early with ET$=$True~(see lines~\ref{line.et 2} and \ref{line.et 1} in \Cref{alg.local detection}). Otherwise, it continues to follow the globally accelerated path with $\mathrm{ET} = \mathrm{False}$.
...
Since our goal here is to analyze the worst-case global complexity, we assume the iterates do not step into \texttt{Track 1} of \Cref{alg.local detection}, in which case it is mostly relevant to local convergence analysis (cf. \Cref{sec.local}). Denote $K_\epsilon$ as the iteration number \Cref{alg.accelerated utr} takes to find an approximate solution satisfying \eqref{eq.approximate solution}. We next show $K_\epsilon = O(\epsilon^{-1/3})$.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
We first demonstrate that the ratio between the regularizer and the step size remains stable for $k\in\mathcal{N}$, as shown in \eqref{eq.output of alg2}. To maintain the flow of the complexity analysis, we temporarily assume the output of \texttt{track 2} of \Cref{alg.local detection} satisfies~\eqref{eq.output of alg2} as well, i.e., \eqref{eq.output of alg2} holds for $k\in\mathcal{Z}$~(we will rigorously verify this later).
\begin{lemma}
...
\begin{lemma}\label{lem.estimate sequence}
    ...
    \end{equation}
    which is a compact form of \eqref{eq.es relation}.
\end{lemma}
\begin{proof}
...
It remains to analyze how many oracles are needed in \Cref{alg.local detection} to find a direction that satisfies \eqref{eq.output of alg2}. Note that \Cref{alg.local detection} contains two tracks, which may be implemented in parallel. As mentioned, \texttt{Track 1}~(see line~\ref{line.track 1 number of iter}) is relevant to local convergence and will be discussed later.
...
\begin{proof}
    ...
    Next, we consider the case \Cref{alg.local detection} enters \texttt{Track 1}. We show that \texttt{Track 1} will find $\epsilon$-approximate solutions in $O(\log\log 1/\epsilon)$ iterations, which is strictly less than the maximal number of iteration we defined at line~\ref{line.track 1 number of iter}, i.e., $\left \lceil\log\frac{\|\nabla f(y)\|}{\epsilon} \right\rceil$.

    We know from \eqref{eq.psd xk} that when $y_k\in\mathcal{LQ}_g$, $\|\nabla f(y_k)\|\leq \frac{1}{C(M,\nu)}$.
    
    If $\frac{1}{2C(M,\nu)}\leq \|\nabla f(y_k)\|\leq \frac{1}{C(M,\nu)}$, from \eqref{eq.regularity epsilon},
    \begin{equation*}
        \begin{aligned}
            \left\lceil\log\log C(M,\nu)/\epsilon+1\right\rceil\leq\log\log C(M,\nu)/\epsilon+2\leq\log\frac{1}{2C(M,\nu)\epsilon}\leq\log\frac{\|\nabla f(y)\|}{\epsilon}.
        \end{aligned}
    \end{equation*}
    Note that the LHS is the number it takes for Newton's method to converge, the RHS is the maximal iteration number.
...
We now turn to another accelerated TR method that incorporates the acceleration framework of \citet{monteiro2013accelerated}. This method achieves a near-optimal global oracle complexity of $\tilde{O}(\epsilon^{-2/7})$ for finding $\epsilon$-approximate solutions. However, this global efficiency comes at the expense of forfeiting local quadratic convergence. We refer to this approach as the Accelerated Trust-Region Extragradient Method (\Cref{alg.ms accelerated utr}).

To analyze the complexity of \Cref{alg.ms accelerated utr}, we impose the following additional assumption:
...
\end{assumption}
\subsection{Algorithm design}
\Cref{alg.ms accelerated utr} outlines the acceleration framework: unlike the original acceleration framework designed primarily for general second-order oracles in \citet{monteiro2013accelerated}, our version introduces an auxiliary parameter $\gamma$ in the update of $v_k$ to accommodate the structure of $\trp$. At the heart of \Cref{alg.ms accelerated utr} lies an implicit search procedure (\Cref{alg.bisection ms}) for determining the primal prior regularization parameter $\sigma_k$. This procedure is customized to fit the $\trp$ and is tightly integrated with the role of $\gamma$ within our framework.
\begin{algorithm}[ht]
...
\begin{algorithm}[!ht]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE{Perform bisection over $\sigma\in [\sigma_-,\sigma_+]$}
        \STATE{
           % \label{eq.ms utr oracle}
                $(\lambda,d) = \trp(y(\sigma),\sigma,\frac{1}{M}\sigma)$
        }
        \ENDWHILE
...
We make several remarks on \Cref{alg.bisection ms}. First, we define the curve for the search procedure given the current $x$ and $v$:
\begin{gather}
                \label{eq.update yk}
...
\end{gather}
We refer to the search procedure as implicit because the base point $y_k$ on which we perform $\trp$ depends on $\sigma_k$ and is updated dynamically along the search; see~\eqref{eq.update yk}. Second, the algorithm initializes the bracketing points for bisection at line~\ref{line.def G0}, where
$$G_0 = \max\{\|\nabla f(x)\| \ \|x-x^*\|\leq \left(\frac{4}{\sqrt{3\gamma}}+1\right)D_0\}, D_0 = \|x_0-x^*\|.$$ 
Finally, \Cref{alg.bisection ms} applies a bisection on $\sigma_k$ at line~\ref{line.goal of bisection ms}, with the goal of finding a value of $\sigma_k$ such that
\begin{equation}
    \label{eq.relation lambda sigma}
    0 \leq \lambda_k \leq (\theta - 1)\sigma_k, \quad \|d_k\| \geq \frac{\eta}{M} \sigma_k,
\end{equation}
where $\theta$ and $\eta$ are prescribed thresholds for the regularization term and the step size, respectively. We remark that at line~\ref{line.goal of bisection ms}, when case i) occurs, we set $\sigma_+ = \sigma$, and when case ii) occurs, we set $\sigma_- = \sigma$.

The condition in~\eqref{eq.relation lambda sigma} can be viewed as a generalized form of the standard stable ratio, which we will further clarify in \Cref{lem.reduce line search}.
...
The second path arises if \Cref{alg.bisection ms} is not early terminated within $K_\epsilon$ iterations. In this case, for each $k$, the step $d_k$ satisfies \eqref{eq.relation lambda sigma}. Consequently, in the end, the recursive relation of estimating sequence guarantees that we will find a solution with a small function value.
\subsection{Global complexity analysis}
\subsubsection{Iteration complexity of \Cref{alg.ms accelerated utr}}
Since we focus on the worst-case global oracle complexity here, we assume that early termination does not occur. Similar to previous analyses, let $K_\epsilon$ denote the iteration number \Cref{def.approximate solution} takes to find an $\epsilon$-approximate solution. We will show that $K_\epsilon = O(\epsilon^{-2/7})$. First, we assume the bisection procedure is effective and \eqref{eq.relation lambda sigma} holds for each $k$~(which will be verified later).
...
\begin{lemma}\label{lem.ms estimate}
    Suppose Assumption~\ref{assm.lipschitz}, Assumption~\ref{assm.solvable} hold, and the output of \Cref{alg.bisection ms} satisfies \eqref{eq.relation lambda sigma}, i.e.,
    \begin{equation*}
...
\end{proof}
From \eqref{eq.ms estimate advanced}, it is clear that the oracle complexity of \Cref{alg.ms accelerated utr} reduces to analyzing the growth rate of $A_k$, a task already addressed in \citet[Lemma 4.2]{monteiro2013accelerated} and \citet[Lemma 4.3.5]{nesterov_lectures_2018}. For conciseness, we defer the full proof to the appendix.
\begin{lemma}
    ...
    \end{equation}
\end{lemma}
Hence, if the bisection procedure is valid, the convergence of \Cref{alg.ms accelerated utr} comes as follows:
...
\end{theorem}
\begin{proof}
    This is a direct consequence of \eqref{eq.ms estimate advanced} and \eqref{eq.growth of Ak}.
\end{proof}
\subsubsection{Oracle complexity of \Cref{alg.ms accelerated utr}}
Next, we elaborate on how \Cref{alg.bisection ms} is guaranteed to make \eqref{eq.relation lambda sigma} hold, and analyze how many oracles are called during this procedure. We omit the subscripts in the analysis for the bisection for simplicity. We now define the crucial function in the analysis, which is a bivariate function of $y$ and $\sigma$. We show that it can serve as a tool to analyze the bisection procedure.
\begin{equation}
...
\end{equation}
We first analyze its basic properties with respect to its variables. We begin by analyzing its property in terms of $\sigma$.
\begin{lemma}
...
\begin{lemma}
    ...
    At line \ref{line.goal of bisection ms} of \Cref{alg.bisection ms}, if case i) happens, i.e.,
    \begin{equation}
        \label{eq.bracket small lambda}
        \lambda = 0, \ \|d\|<\frac{\eta}{M}\sigma,
    \end{equation}
    then $\psi(\sigma,y(\sigma))<\frac{\eta}{M}$.  Otherwise, if case ii) happens, i.e.,
    \begin{equation}
...
\end{proof}
The above \Cref{lem.reduce line search} means we can simplify the analysis for the line search problem to the one based on the auxiliary function $\psi(\sigma,y)$, whose form looks quite related to the stable ratio condition.
...
\begin{lemma}
    ...
    Suppose \Cref{assm.lipschitz} holds and \Cref{alg.bisection ms} outputs ET$=$True, we have
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
\end{proof}
Next, we check the validity of the bracketing points.
\begin{lemma}
    ...
    \end{equation}
    If \Cref{alg.bisection ms} outputs ET$=$False, we have
    \begin{equation*}
...
\begin{proof}
    ...
    From \Cref{lem.early termination ms}, we can conclude that if ET$=$False, we have $\lambda_- > (\theta-1)\sigma_-$, from \Cref{lem.reduce line search} we can conclude $\psi(\sigma_-,y(\sigma_-))>\frac{1}{M}.$
\end{proof}
After finding the bracket points, we are now ready to analyze the complexity of the bisection procedure. First, we need to introduce the following property of $y(\cdot)$, since it is an auxiliary lemma, and the proof is almost the same as in \citet{monteiro2013accelerated}, we delay its proof to \Cref{sec.appendix 2}.
\begin{lemma}[Lemma 7.13, \citet{monteiro2013accelerated}]
    ...
    \end{equation}
\end{lemma}
To summarize our derivation so far: if \eqref{eq.bounded gradient y} and \eqref{eq.bounded assm point} hold, then, provided the bisection procedure does not terminate, we have
\begin{gather}
...
\end{gather}
Following some standard analysis for bisection, we can derive the complexity of \Cref{alg.bisection ms}.
\begin{lemma}
    \label{lem.complexity bisection}
    Assume that Assumptions~\ref{assm.lipschitz} and \ref{assm.solvable} are satisfied, and that conditions \eqref{eq.bounded gradient y} and \eqref{eq.bounded assm point} also hold, then the number of $\trp$ oracle calls during the bisection is $O(\log 1/\epsilon)$. 
\end{lemma}
We put a more formal presentation and proof of \Cref{lem.complexity bisection} in \Cref{sec.appendix 2}~(\Cref{coro.bisection bound}).
...
\end{proof}
By this lemma, we know \eqref{eq.bounded gradient y} holds, and \eqref{eq.bounded assm point} holds with $M_0 =\left(\frac{4}{\sqrt{3\gamma}}+1\right)D_0 $. Now we finally come to the oracle complexity of \Cref{alg.ms accelerated utr}.
\begin{theorem}
...
\end{theorem}
\begin{proof}
    This is a consequence of \Cref{thm.outer loop complexity}, \Cref{lem.complexity bisection} and \Cref{lem.bounded iterate}.
\end{proof}

\section{Numerical Experiments}\label{sec.experiments}
In this section, two different accelerated trust-region methods using the $\trp{}$ oracle (\atr{} and \atrms{}) are implemented, and compared to some state-of-the-art SOMs:
\begin{itemize}[leftmargin=*]
    \item The cubic regularization method (\arc{}, \citet{nesterov2006cubic}) \cz{adaptive or not? If not, we should clarify how to choose M. Similarly, how to choose M in the accelerated cubic? Also, I think the subproblem solver varies from different methods. We should clarify our subproblem solver in detail and briefly introduce the one used in cubic newton} and its accelerated version (\arca{}, \citet{nesterov2008accelerating}).
    \item A non-accelerated trust-region method using the $\trp{}$ oracle, by setting both $(\sigma_k, r_k)$ proportionally to $\|\nabla f(x_k)\|^{1/2}$. This method is termed \utr{} in \cite{jiang2024nonconvexityuniversaltrustregionmethod}.
\end{itemize}
...
As mentioned, this problem is a notorious example where many accelerated SOMs are not as competitive as the classical Newton-type methods \cite{carmon2022optimal,jiang2024accelerated,chen2022accelerating} numerically.
...
We could have several observations. Firstly, for non-accelerated methods, a trend of local superlinear convergence can be observed in \utr{}~\texttt{(1)}, \utr{}~\texttt{(2)} and \arc{}.\cz{we don't observe that for \utr{}~\texttt{(2)}} We could conclude that \utr{} and \arc{} are comparable.
Secondly, in the beginning of the iterations, all accelerated methods, including \atr{}, \atrms{}, and \arca{}, converge faster than the non-accelerated methods (e.g., \utr{}~\texttt{(1)}, \utr{}~\texttt{(2)}, \arc{}). This confirms the effectiveness of the acceleration techniques. However, only \atr{} has the local rate of superlinear convergence. Both \atrms{} and \arca{} converge sublinearly in the local regime, in which \atrms{} is slightly better because of its superior non-asymptotic performance. These results are in accordance with what is predicted in the convergence analysis.
...
\section{Technical Proofs in Section~\ref{sec.second variant}}\label{sec.appendix 2}
...
\subsection*{Proof to \Cref{lem.psi property wrt y}}
\begin{proof}
    We denote
    \begin{gather*}
...
\end{proof}
\subsection*{Proof to \Cref{lem.curve condition}}
\begin{proof}
    We have
    \begin{equation*}
        \begin{aligned}
...
In fact, we prove a more detailed version of \Cref{lem.complexity bisection} here, which is \cref{coro.bisection bound} here. First, we need the following auxiliary lemma below.
\begin{lemma}
