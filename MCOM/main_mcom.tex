...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE \cz{$(x_{k+1},\text{ET}) = \operatorname{LD}(y_k,\tfrac{\sqrt{2M}}{2}\|\nabla f(y_k)\|^{1/2},d_k,\epsilon)$}
...
\begin{subroutine}[!ht]
    ...
    \begin{algorithmic}[1]
        ...
        \ENDWHILE
        \STATE{\textbf{output} None, $d$, $\mu$, ET= False}
        \STATE{\cz{\textbf{output} $y + d$, ET= False} }
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
We first examine the ratio between the regularizer and the step size in the first case.
\begin{lemma}
...
Now we focus on the oracle complexity of \texttt{Track 2}, i.e., the complexity of bisection over radius $r$ on the interval $[r_-,r_+]$ to make \eqref{eq.output of alg2} hold (see line~\ref{line.left bracketing point}). For simplicity, we omit the subscript $k$ in the analysis for bisection.
...
\end{equation}
This motivates us to introduce the following auxiliary functions of $\mu$.
\begin{equation}
...
\end{equation}
Given the base point $y$, the goal of bisection to locate the radius $r$ can be represented as,
\begin{equation*}
    M \leq g_y(\mu) \leq 2M.
\end{equation*}
\cz{[remove this]
    We remark that $g_y(\mu)$ is also function of $r$ given \eqref{eq.auxiliary function for bisection 2}.}
The logic of our proof is to first identify the target interval of $\mu$ and then use the one-to-one correspondence to transfer it to the target interval of $r$.
The following lemma validates the choices of $g_y(\mu)$ at line~\ref{line.left bracketing point}.
\begin{lemma}
