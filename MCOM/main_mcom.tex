...
In this section, we present the numerical experiments of the proposed methods. All experiments are conducted on a single machine with a 14-core Apple M4 Pro CPU and 48GB LPDDR5 RAM.
...
This bound is quite conservative according to \cite{songUnifiedAccelerationHighorder2021}.
However, we use it to single out from other algorithmic improvements such as the adaptive adjustments of Lipschitz constants, which are now common in the literature (e.g., \cite{mishchenko_regularized_2023,cartis2011adaptive1}).

Using this fixed estimate, we implement two different accelerated trust-region methods using the $\trp{}$ oracle (\atr{} and \atrms{}) and compared to some state-of-the-art SOMs:
\begin{itemize}[leftmargin=*]
    ...
    \item A non-accelerated trust-region method using the $\trp{}$ oracle, by setting both $(\sigma_k, r_k)$ proportionally to $\|\nabla f(x_k)\|^{1/2}$ similar to \cite{jiang2024nonconvexityuniversaltrustregionmethod}.
          We test two non-accelerated trust-region methods $\utr{}~\texttt{(1)}, \utr{}~\texttt{(2)}$, using different Lipschitz estimates $\tfrac{\widehat M}{2}, \widehat M$, respectively. The purpose is to present the sensitivity of Lipschitz constants and set a fair comparison to \arc{}.
\end{itemize}
The subproblems arised in \arc{} and \arca{} are solved by a 1-D line-search strategy according to \cite{nesterov_lectures_2018}. Similarly, $\trp{}$ (in \utr{}, \atr{} and \atrms{}) is solved by searching the dual variable. All methods use exact Hessian evaluation and Cholesky factorization to solve the linear systems.
Since these SOMs use different subproblems, and theoretically, the complexity rates to solve them vary from $O(\log(\epsilon))$ (for subproblems in a cubic regularized method \cite{nesterov2006cubic}) to $O(\log\log(\epsilon))$ (for $\trp{}$ \cite{yenewcomplexityresult1991,vavasisProvingPolynomialtimeSphereconstrained1990}),
we report the number of Hessian evaluations needed in the method.  In \Cref{fig.logistic regression}, the performance of the SOMs on a few LIBSVM datasets\footnote{For details, see \url{https://www.csie.ntu.edu.tw/cjlin/libsvmtools/datasets/}} are reported.
\begin{figure}[h]
    \small
    \centering
...
Secondly, in the beginning of the iterations, all accelerated methods, including \atr{}, \atrms{}, and \arca{}, converge faster than the non-accelerated methods (e.g., \utr{}~\texttt{(1)}, \utr{}~\texttt{(2)}, \arc{}). This confirms the effectiveness of the acceleration techniques.

However, the global convergence comes at an expense of local superlinear convergence.
Notably, \atr{} is the only accelerated method that has the local rate of superlinear convergence.
