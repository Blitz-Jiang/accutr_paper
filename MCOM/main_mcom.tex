...
The above lemma shows that we can bound the gradient norm of the next iterate by controlling $\mu$.
...
\cz{
    ...
    To balance the global-local performance, we introduce the $\trp{}$ oracle here formally again for ease of reading.
}
...
It remains to analyze how many oracles are needed in \Cref{alg.local detection} to find a direction that satisfies \eqref{eq.output of alg2}. Note that \Cref{alg.local detection} contains two tracks which are implemented in parallel. For \texttt{Track 1}, it is obvious that we need to solve at most $O(\log \frac{\|\nabla f(y_k)\|}{\epsilon})$ linear equations. We will see the number of $\trp$ oracles needed to find the $\epsilon$-approximate solutions is bounded by $\tilde{O}(\epsilon^{-1/3})$. We remark that the additional logarithmic number of TR oracles are called as a price for the local information at the current solution.
...
Now we focus on the oracle complexity of \texttt{Track 2}, i.e., the complexity of bisection search over radius $r$ on the interval $[r_-,r_+]$ to make \eqref{eq.output of alg2} hold (see line~\ref{line.left bracketing point}). For simplicity, we omit the subscript $k$ in the analysis for bisection.

In each step of the bisection, one oracle $(d,\mu)=\trp(y,0,r)$ with $r\in[r_-,r_+]$ is called, which gives
\begin{equation}
...
\begin{lemma}
    ...
    On \texttt{Track 2} of \Cref{alg.local detection}, we set
    \begin{equation*}
        r_- = \|d_+\|, \ r_+ = \left \|\left(\nabla^2f(y)+M\|d_+\| I\right)^{-1}\nabla f(y)\right\|,
    \end{equation*}
...
\end{equation}
It means the target interval for $\mu$ is $[\mu_l,\mu_u]$. Now we show that the length of the target interval for $\mu$ is bounded below.
\begin{lemma}
...
\end{proof}
With the above analyses for the auxiliary function, the complexity of \texttt{Track 2} is bounded as follow.
\begin{lemma}
