...
In this paper, we answer the above question by developing new accelerated SOMs and, at the same time, highlighting the tradeoffs between global and local efficiency.
...
Two distinct strengths make \eqref{eq.classic tr subproblem} a compelling candidate for answering the fundamental question. On the theoretical side, \tr{} oracles can be solved in $O(n^3 + n^2\log\log(\epsilon^{-1}))$ arithmetic operations \cite{vavasisProvingPolynomialtimeSphereconstrained1990,yenewcomplexityresult1991}, incurring no significant overhead than a Newton equation.
Moreover, when the ball constraint becomes inactive (which can be detected by $\lambda$ being zero) near a non-degenerate minimizer, the step reduces to the pure Newton direction, automatically revealing a region of quadratic convergence --- an advantage absent from other second-order oracles. This unique feature offers a possibility for local detection and exploitation.
...
However, how to use TR oracles introduces a central technical challenge. 
\cz{While the dual variable $\lambda$ encodes the geometry that allows local convergence, it is a double-edged sword that complicates the analysis for global efficiency. In fact, classical TR methods cannot match the global complexity of unaccelerated SOMs in convex optimization.} \cz{Maybe add a footnote saying that it is no better than GD; Cartis said so}. This is because $\lambda$ is an a posteriori estimate and so requires a monitoring process \cite{curtis2017trust}. 
\cz{For short, one is to stabilize the dual, another one is to make use of the ``local detection'' feature of TR. At the same time, why can't NPE use this?}
...
We introduce new strategies for adjusting the key parameters $\sigma$ (regularization strength) and $r$ (trust-region radius) in the TR oracle, which we call trust-region oracle for acceleration~(denote as $\trp$):
\begin{subequations}\label{eq.newTR}
    ...
    \end{align}
\end{subequations}
\cz{We denote $(d,\lambda) = \trp(x,\sigma,r)$ as the primal-dual solution of this problem. Maybe we do not have to present $\trp$}
