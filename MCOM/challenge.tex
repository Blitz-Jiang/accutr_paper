...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where \( \Delta \) denotes the trust-region radius. For an overview of TR methods, we refer the readers to two excellent monographs \cite{yuan2000review,yuan2015recent}.
...
However, using TR oracles introduces a central technical challenge: the Lagrange multiplier introduced by the TR constraint acts as a \textit{double-edged sword}.
...
The first method, \emph{Global-Local Accelerated Trust-Region Method (GL-ATR)} (Algorithm~\ref{alg.accelerated utr}), implements a local detection mechanism (Algorithm~\ref{alg.local detection}) that automatically identify quadratic convergence regions. This design yields an improved global worst-case oracle complexity of $\tilde{O}(\epsilon^{-1/3})$, while achieving a local quadratic rate of convergence. To our knowledge, this is the first accelerated SOM that provably achieves both $\tilde{O}(\epsilon^{-1/3})$ global complexity and local quadratic convergence.

The second method, \emph{Near-Optimal Accelerated Trust-Region Method (NO-ATR)} (Algorithm~\ref{alg.ms accelerated utr}), is designed to push the limits of global performance, reaching the near-optimal global oracle complexity of $\tilde{O}(\epsilon^{-2/7})$ for convex problems. This is achieved by implementing an implicit search procedure tailored to $\trp$ oracles (Algorithm~\ref{alg.bisection ms}). However, our analysis and experiments reveal that this global optimality comes at the cost of losing local efficiency because of the inability to incorporate local detection mechanisms. 
...
In contrast, our work focuses on the general convex problem~\eqref{eq.main problem} with a non-degenerate minimizer, without assuming global strong convexity. Our method initiates local detection automatically when the constraint becomes inactive, rather than relying on restart mechanisms or prior knowledge of strong convexity parameters. As a result, it applies to a significantly broader class of convex problems.

% \cz{2. review the TR literature}
From a historical perspective, despite the strong empirical success of TR methods, the global complexity analyses remain incomplete. This gap has motivated several early efforts that focus on nonconvex problems~\cite{yeSecondOrderOptimization2005,curtis2017trust,curtis2021trust,zhangHomogeneousSecondorderDescent2025,hamad2022consistently,grapiglia2015convergence}. 
